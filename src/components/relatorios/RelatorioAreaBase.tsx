...
import FiltrosEspecificos from '@/components/relatorios/FiltrosEspecificos';
import { carregarDadosRelatorio } from '@/lib/relatorios/carregarDados';
        ...
        switch (areaId) {
          case 'vendas':
            const [clientesRes, vendedoresRes] = await Promise.all([
              fetch(`/api/cadastros?company_id=${activeCompanyId}&tipo=cliente`, { headers }),
              fetch(`/api/cadastros?company_id=${activeCompanyId}&tipo=vendedor`, { headers })
            ]);
            if (clientesRes.ok) {
          ...
          case 'estoque':
            const [locaisRes, produtosRes] = await Promise.all([
              fetch(`/api/estoque/locais?company_id=${activeCompanyId}`, { headers }),
              fetch(`/api/produtos?company_id=${activeCompanyId}`, { headers })
            ]);
            if (locaisRes.ok) {
