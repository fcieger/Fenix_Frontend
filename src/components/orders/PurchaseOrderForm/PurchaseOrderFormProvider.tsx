...
import type { PurchaseOrder, PurchaseOrderItem } from "@/types/sdk";
...
export interface PurchaseOrderFormContextValue {
  ...
  updateItem: (id: string, item: Partial<PurchaseOrderItem>) => void;
...
export interface PurchaseOrderFormProviderProps {
  children: ReactNode;
  orderId?: string;
}
...
export function PurchaseOrderFormProvider({
  ...
  const [items, setItemsState] = useState<PurchaseOrderItem[]>([]);
  ...
  const setItems = useCallback((newItems: PurchaseOrderItem[]) => {
  ...
  const value: PurchaseOrderFormContextValue = {
    order: order || null,
    items,
    totals,
    isLoading,
...
export function usePurchaseOrderFormContext() {
  const context = useContext(PurchaseOrderFormContext);
  if (context === undefined) {
    throw new Error(
      "usePurchaseOrderFormContext must be used within a PurchaseOrderFormProvider"
    );
  }
  return context;
}
