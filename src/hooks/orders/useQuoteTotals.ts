...
import type { OrderTotals } from "@/components/orders/QuoteForm/QuoteFormProvider";

/**
 * Hook para calcular totais do orçamento
 * Calcula subtotal, impostos, descontos e total final
 */
export function useQuoteTotals() {
  const { items, totals, setTotals, recalculateTotals } = useQuoteFormContext();
