...
import { listarPedidosVenda, excluirPedidoVenda, obterPedidoVenda } from '@/services/sales-orders-service';
...
export default function PedidosVendaPage() {
  ...
  // Helper function to load partner for a sales order
  const loadPartnerForOrder = async (order: SalesOrder): Promise<Partner | null> => {
  ...
  useEffect(() => {
    const loadPedidoVendas = async () => {
      try {
        ...
        // SDK returns PaginatedResponse<SalesOrder> with { data: SalesOrder[], meta: {...} }
  ...
  const stats = {
    ...
    finalizados: pedidoVendas.filter(o => o.status === OrderStatus.COMPLETED).length,
    pendentes: pedidoVendas.filter(o => o.status === OrderStatus.IN_PROGRESS).length,
    rascunhos: pedidoVendas.filter(o => o.status === OrderStatus.DRAFT).length,
    valorTotal: pedidoVendas.reduce((acc, o) => acc + (o.total || 0), 0)
  };
    ...
    <Layout>
      <div className="space-y-6">
        ...
        {isLoadingPedidoVendas ? (
          ...
          >
            {currentPedidoVendas.map((order, index) => {
              const partner = partnersCache.get(order.partnerId || "");
              const displayOrder = mapSalesOrderToDisplay(order, partner || undefined);
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex gap-2">
                        ...
                        <motion.button
                          whileHover={{ scale: 1.05 }}
                          whileTap={{ scale: 0.95 }}
                          onClick={() => handleExportarPDF(order.id, displayOrder.number)}
            ...
            <div className="overflow-x-auto">
              <table className="min-w-full divide-y divide-gray-200">
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {currentPedidoVendas.map((order, index) => {
                    const isExpanded = expandedPedidoVenda === order.id;
                    const detalhes = pedidoVendaDetalhes[order.id];
                    const partner = partnersCache.get(order.partnerId || "");
                    const displayOrder = mapSalesOrderToDisplay(order, partner || undefined);
                            ...
                            <div className="space-y-6">
                              ...
                              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                                <div className="bg-white p-4 rounded-lg shadow-sm">
                                  ...
                                  {(() => {
                                    ...
                                    return partner?.tradeName ? (
                                      <p className="text-xs text-gray-500 mt-1">{partner.tradeName}</p>
                                    ) : null;
                                  })()}
                                </div>
                                ...
                                <div className="bg-white p-4 rounded-lg shadow-sm">
                                  ...
                                  <p className="text-sm text-gray-900">
                                    {detalhes.date ? new Date(detalhes.date).toLocaleDateString('pt-BR') : '-'}
                                  </p>
                                </div>
                              </div>
                                      ...
                                      <tbody className="bg-white divide-y divide-gray-200">
                                        {detalhes.itens.map((item: any, idx: number) => (
                                          <tr key={item.id || idx} className="hover:bg-gray-50">
                                            ...
                                            <td className="px-4 py-2 text-sm text-gray-900 text-right">{Number(item.quantity || 0).toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</td>
                                            <td className="px-4 py-2 text-sm text-gray-900 text-right">{formatCurrency(Number(item.unitValue || 0))}</td>
                                            <td className="px-4 py-2 text-sm text-gray-900 text-right">{formatCurrency(Number(item.discount || 0))}</td>
                                            <td className="px-4 py-2 text-sm font-medium text-gray-900 text-right">{formatCurrency(Number(item.total || 0))}</td>
                                          </tr>
                                        ))}
                                      </tbody>
