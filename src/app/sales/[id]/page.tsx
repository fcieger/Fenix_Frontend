...
function PedidoVendaFormPage() {
  ...
  useEffect(() => {
    const loadInitialData = async () => {
      ...
      try {
        if (id && id !== 'novo') {
          const pedidoVenda = await obterPedidoVenda(id) as SalesOrder;

          // Pedido de Vendas finalizados não podem ser editados (status COMPLETED)
          if (pedidoVenda.status === 'COMPLETED') {
          ...
          setModel(pedidoVenda as any); // Keep model for backward compatibility

          // Buscar orçamento origem se existir
          if (pedidoVenda.quoteId) {
            buscarOrcamentoOrigem(pedidoVenda.quoteId);
          }

          // Usar função helper para mapear SalesOrder do SDK para formData
          const formDataMapped = mapSdkSalesOrderToFormData(pedidoVenda, activeCompanyId);
          ...
          // Preencher itens - mapear SalesOrderItem do SDK para formato do formulário
          const itensMapped = (pedidoVenda.items || []).map((it: any, index: number) => ({
            id: it.id || `item-${index}`,
            companyId: pedidoVenda.companyId,
            produtoId: it.productId || null,
            naturezaOperacaoId: it.operationNatureId || pedidoVenda.operationNatureId || '',
            codigo: it.product?.code || '',
            nome: it.product?.description || '',
            unidade: 'UN', // Not in SalesOrderItem, default
            ...
            ncm: '', // Not in SalesOrderItem, would need to fetch from Product
            cest: '', // Not in SalesOrderItem
            observacoes: '', // Not in SDK
            numeroItem: String(index + 1),
            // Campos de impostos - não estão no SalesOrderItem, mas podem estar calculados
            icmsBase: undefined,
              ...
              <CardContent className="p-4">
                <div className="flex items-center space-x-3">
                  ...
                  <div className="flex-1">
                    ...
                    <button
                      onClick={() => router.push(`/quotes/${orcamentoOrigem.id}`)}
            ...
            <div className="max-w-7xl mx-auto px-4 py-4">
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center space-x-3">
                  ...
                  <Button
                    onClick={() => router.push('/sales/novo')}
                    variant="outline"
