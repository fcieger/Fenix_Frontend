...
function NovoProdutoForm() {
  ...
  useEffect(() => {
    ...
    if (edit === 'true' && id && data) {
      try {
        ...
        // Preencher formData com os dados do produto (mapeando campos do SDK)
        const product = produtoData as Product;
        setFormData({
          ...
          origem: '0',
          precoCusto: '',
          precoVenda: product.price ? product.price.toString() : '',
  ...
  const handleUpdate = async (updateData: UpdateProductDto) => {
    ...
    try {
      ...
      openSuccess({ title: 'Produto atualizado', message: 'Produto atualizado com sucesso.', onClose: () => router.push('/products') });
    } catch (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // SDK CreateProductDto requer: code, description, ncm, unit, price
      // cest é opcional
      const produtoData: CreateProductDto = {
        code: formData.codigo?.trim() || formData.nome.trim().substring(0, 20).toUpperCase().replace(/\s/g, '_'),
        description: formData.nome.trim(),
        ncm: formData.ncm?.trim() || '',
        cest: formData.cest?.trim() || undefined,
        unit: formData.unidadeMedida || 'UN',
        price: formData.precoVenda ? parseFloat(formData.precoVenda.replace(/\D/g, '')) / 100 : 0,
      };

      if (isEditMode && editId) {
        // Modo de edição - atualizar produto usando SDK service
        await updateProduct(editId, produtoData);
        openSuccess({ title: 'Produto atualizado', message: 'Produto atualizado com sucesso.', onClose: () => router.push('/products') });
      } else {
        // Modo de criação - criar novo produto usando SDK service
        await createProduct(produtoData);
        openSuccess({ title: 'Produto salvo', message: 'Produto criado com sucesso.', onClose: () => router.push('/products') });
      }
    } catch (error) {
