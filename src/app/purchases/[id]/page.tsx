...
import TabsCompra from '@/components/purchases/tabs-compra';
import EditCadastroModal from '@/components/purchases/EditCadastroModal';
...
import { obterPedidoCompra, criarPedidoCompra, atualizarPedidoCompra, recalcularImpostos, entregarPedidoCompra } from '@/services/purchase-orders-service';
...
function PedidoCompraFormPage() {
  ...
  useEffect(() => {
    const loadInitialData = async () => {
      ...
      try {
        if (id && id !== 'novo') {
          const pedidoCompra = await obterPedidoCompra(id, activeCompanyId ? activeCompanyId : undefined) as PurchaseOrder;

          // Pedidos de Compra finalizados não podem ser editados (status COMPLETED ou DELIVERED)
          if (pedidoCompra.status === 'COMPLETED' || pedidoCompra.status === 'DELIVERED') {
          ...
          setModel(pedidoCompra as any); // Keep model for backward compatibility

          // Usar função helper para mapear PurchaseOrder do SDK para formData
          const formDataMapped = mapSdkPurchaseOrderToFormData(pedidoCompra);
          ...
          // Preencher itens - mapear PurchaseOrderItem do SDK para formato do formulário
          const itensMapped = (pedidoCompra.items || []).map((it: any, index: number) => ({
            id: it.id || `item-${index}`,
            companyId: pedidoCompra.companyId,
            produtoId: it.productId || null,
            naturezaOperacaoId: it.operationNatureId || pedidoCompra.operationNatureId || '',
            codigo: it.product?.code || '',
            nome: it.product?.description || '',
            unidade: 'UN', // Not in PurchaseOrderItem, default
            ...
            ncm: '', // Not in PurchaseOrderItem, would need to fetch from Product
            cest: '', // Not in PurchaseOrderItem
            observacoes: '', // Not in SDK
            numeroItem: String(index + 1),
            // Campos de impostos - não estão no PurchaseOrderItem, mas podem estar calculados
            icmsBase: undefined,
          ...
          if (pedidoCompra.partnerId) {
            const f = cadastros.find((f: any) => f.id === pedidoCompra.partnerId);
            if (f) setFornecedorSelecionado(f);
          }
          // compradorId e transportadoraId não existem no SDK

          if (pedidoCompra.paymentTermId) {
            const p = prazosPagamento.find((p: any) => p.id === pedidoCompra.paymentTermId);
            if (p) setPrazoSelecionado(p);
          }

          // formaPagamentoId e localEstoqueId não existem no SDK
        }
      } catch (err: any) {
    ...
    try {
      ...
      if (isNovo) {
        ...
        openSuccess({
          ...
          onClose: () => {
            router.replace(`/purchases/${saved.id}`);
          }
        });
      } else {
            ...
            <div className="max-w-7xl mx-auto px-4 py-4">
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center space-x-3">
                  ...
                  <Button
                    onClick={() => router.push('/purchases/novo')}
                    variant="outline"
