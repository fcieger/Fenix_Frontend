...
export default function LocaisEstoquePage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/estoque/locais?${params.toString()}`, { cache: "no-store" });
  ...
  const submit = async () => {
    ...
    try {
      ...
      let res: Response;
      if (editId) {
        res = await fetch(`/api/estoque/locais`, {
          method: "PUT",
  ...
  const remove = async (id: string) => {
    ...
    const res = await fetch(`/api/estoque/locais?id=${id}`, { method: "DELETE" });
    const json = await res.json();
    if (!json?.success) {
  ...
  const setAsDefault = async (localId: string) => {
    ...
    try {
      const res = await fetch('/api/estoque/locais/default-company', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
                    ...
                    <div className="flex items-end">
                      <Button 
                        onClick={async ()=>{
                          ...
                          const res = await fetch('/api/estoque/locais/default-company', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ companyId: configCompanyId, localId: defaultCompanyLocalId })});
                    ...
                    <div className="flex items-end">
                      <Button 
                        onClick={async ()=>{
                          ...
                          const res = await fetch('/api/estoque/produtos/local-padrao', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ produtoId: produtoPadraoProdutoId, localId: produtoPadraoLocalId })});
                          const j = await res.json();
