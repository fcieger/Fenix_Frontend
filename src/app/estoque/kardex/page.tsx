...
export default function KardexPage() {
  ...
  const loadMovimentos = useCallback(async (produtoIdParam?: string) => {
    ...
    try {
      ...
      const res = await fetch(`/api/estoque/movimentos?${params.toString()}`, { cache: "no-store" });
      const json = await res.json();
      ...
      const resSaldo = await fetch(`/api/estoque/movimentos?${paramsSaldo.toString()}`, { cache: "no-store" });
      ...
      const resSaldos = await fetch(`/api/estoque/saldos?${paramsResumo.toString()}`, { cache: "no-store" });
