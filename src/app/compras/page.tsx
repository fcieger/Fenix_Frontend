...
export default function PedidosCompraPage() {
  ...
  const [pedidoCompraDetalhes, setPedidoCompraDetalhes] = useState<{[key: string]: any}>({});
  ...
  useEffect(() => {
    const loadPedidosCompra = async () => {
      try {
        ...
        setError(null);
        // Passar company_id se disponível
        const params = activeCompanyId ? { company_id: activeCompanyId } : {};
        const result = await listarPedidosCompra(params);
        ...
        const mapped = (resultArray || []).map((o: any) => ({
          ...
          fornecedor: o.fornecedor?.nomeRazaoSocial || o.cliente?.nomeRazaoSocial || '-',
          numero: o.numero || `#${o.id.slice(0, 8)}`,
          comprador: o.comprador?.nomeRazaoSocial || '-',
  ...
  const filteredPedidosCompra = pedidosCompra.filter(pv => {
    const matchesSearch = pv.fornecedor?.toLowerCase().includes(searchTerm.toLowerCase()) ||
      pv.numero?.includes(searchTerm) ||
      pv.comprador?.toLowerCase().includes(searchTerm.toLowerCase());
  ...
  const stats = {
    ...
    finalizados: pedidosCompra.filter(o => o.status === 'Finalizado').length,
    pendentes: pedidosCompra.filter(o => o.status === 'Pendente').length,
    rascunhos: pedidosCompra.filter(o => o.status === 'Rascunho').length,
    valorTotal: pedidosCompra.reduce((acc, o) => acc + o.valorTotal, 0)
  };
    ...
    <Layout>
      <div className="space-y-6">
        ...
        {isLoadingPedidosCompra ? (
          ...
          >
            {currentPedidosCompra.map((pv, index) => (
              <motion.div
                key={pv.id}
                initial={{ opacity: 0, y: 20 }}
                animate={{ opacity: 1, y: 0 }}
                ...
                <div className="p-6">
                  <div className="flex items-start justify-between mb-4">
                    <div>
                      ...
                      <p className="text-sm text-gray-600">{pv.fornecedor}</p>
                    </div>
            ...
            <div className="overflow-x-auto">
              <table className="min-w-full divide-y divide-gray-200">
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {currentPedidosCompra.map((pv, index) => {
                    const isExpanded = expandedPedidoCompra === pv.id;
                    const detalhes = pedidoCompraDetalhes[pv.id];
                    ...
                    return (
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="text-sm text-gray-900">{pv.fornecedor}</div>
                        <div className="text-sm text-gray-500">{pv.comprador}</div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        {getStatusBadge(pv.status)}
                      </td>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                        <div className="flex gap-2 items-center">
                          ...
                          <motion.button
                            whileHover={{ scale: 1.05 }}
                            whileTap={{ scale: 0.95 }}
                            onClick={() => handleDelete(pv.id, pv.fornecedor)}
                            className="flex items-center justify-center w-10 h-10 rounded-xl bg-red-50 hover:bg-red-100 text-red-600 border border-red-200 hover:border-red-300 shadow-sm hover:shadow-md transition-all duration-200 group"
                            title="Excluir"
                          >
                            <Trash2 className="w-4 h-4 group-hover:scale-110 transition-transform" />
