...
import HeaderCompra from '@/components/compras/header-compra';
import ConfiguracaoCompra from '@/components/compras/configuracao-compra';
import ListaProdutos from '@/components/compras/lista-produtos';
import TabsCompra from '@/components/compras/tabs-compra';
import EditCadastroModal from '@/components/compras/EditCadastroModal';
...
import { obterPedidoCompra, criarPedidoCompra, atualizarPedidoCompra, recalcularImpostos, entregarPedidoCompra } from '../../../services/pedidos-compra';
...
function PedidoCompraFormPage() {
  ...
  const [compradorSelecionado, setCompradorSelecionado] = useState<any | null>(null);
  ...
  const [pedidoCompraBloqueado, setPedidoCompraBloqueado] = useState(false);
  const [originalPedidoCompra, setOriginalPedidoCompra] = useState<any>(null);
  ...
  const [model, setModel] = useState<PedidoCompra>(novoPedidoCompra());
  const [loading, setLoading] = useState(!isNovo);

  // formData completo com todos os campos da tela de compras
  ...
  // Recarregar dados do fornecedor quando voltar da página de cadastros
  useEffect(() => {
    const handleMessage = async (event: MessageEvent) => {
      if (event.data?.type === 'cadastroUpdated' && event.data?.id === model.fornecedorId) {
        // Recarregar dados do fornecedor
        try {
          const fornecedorAtualizado = await apiService.getCadastro(event.data.id, token || '');
          if (fornecedorAtualizado) {
            // Atualizar fornecedor selecionado
            setFornecedorSelecionado(fornecedorAtualizado);
            // Atualizar na lista de fornecedores
            setFornecedores(prev => prev.map(f => f.id === event.data.id ? fornecedorAtualizado : f));
          }
        } catch (e) {
          console.error('Erro ao recarregar dados do fornecedor:', e);
        }
      }
    };
  ...
  const loadCadastros = async () => {
    ...
    try {
      ...
      // Separar fornecedors, compradores e transportadoras
      const fornecedorsData = response.filter((c: any) => c.tiposCliente?.fornecedor);
      const compradoresData = response.filter((c: any) => c.tiposCliente?.comprador);
      const transportadorasData = response.filter((c: any) => c.tiposCliente?.transportadora);
      
      setFornecedores(fornecedorsData);
  ...
  const recalcImpostos = async (naturezaId?: string) => {
    ...
    try {
      ...
      if (fornecedorSelecionado?.enderecos?.length > 0) {
        const enderecoPrincipal = fornecedorSelecionado.enderecos.find((e: any) => e.principal) || fornecedorSelecionado.enderecos[0];
      ...
      const payload = {
        ...
        fornecedorId: formData.fornecedor || null,
        naturezaOperacaoId,
  ...
  // Carregar dados iniciais e, se houver id, carregar pedido de compra para edição
  useEffect(() => {
    const loadInitialData = async () => {
      ...
      // Se estiver editando um pedido de compra, carregar dados
      try {
        if (id && id !== 'novo') {
          const pedidoCompra = await obterPedidoCompra(id, activeCompanyId ? activeCompanyId : undefined);
          
          // Pedidos de Compra finalizados não podem ser editados
          if (pedidoCompra.status === 'faturado' || pedidoCompra.status === 'entregue') {
            setPedidoCompraBloqueado(true);
            warning('Pedido de Compra Finalizado', 'Este pedido de compra foi finalizado e não pode mais ser editado.');
          }
          
          setOriginalPedidoCompra(pedidoCompra);
          setModel(pedidoCompra);
          
          // Preencher formData com todos os campos salvos
          setFormData({
            fornecedor: pedidoCompra.fornecedorId || '',
            comprador: pedidoCompra.compradorId || '',
            transportadora: pedidoCompra.transportadoraId || '',
            consumidorFinal: pedidoCompra.consumidorFinal ?? false,
            indicadorPresenca: pedidoCompra.indicadorPresenca || '2',
            formaPagamento: pedidoCompra.formaPagamentoId || '',
            parcelamento: pedidoCompra.parcelamento || '90 dias',
            estoque: pedidoCompra.localEstoqueId || '',
            pedido: pedidoCompra.numero || '',
            nfe: pedidoCompra.serie || '',
            numeroOrdemCompra: pedidoCompra.numeroOrdemCompra || '',
            dataPrevisao: pedidoCompra.dataPrevisaoEntrega 
              ? (typeof pedidoCompra.dataPrevisaoEntrega === 'string' 
                  ? pedidoCompra.dataPrevisaoEntrega.split('T')[0] 
                  : new Date(pedidoCompra.dataPrevisaoEntrega).toISOString().split('T')[0])
              : '',
            dataEmissao: pedidoCompra.dataEmissao 
              ? (typeof pedidoCompra.dataEmissao === 'string' 
                  ? pedidoCompra.dataEmissao.split('T')[0] 
                  : new Date(pedidoCompra.dataEmissao).toISOString().split('T')[0])
              : new Date().toISOString().split('T')[0],
            dataEntrega: pedidoCompra.dataEntrega 
              ? (typeof pedidoCompra.dataEntrega === 'string' 
                  ? pedidoCompra.dataEntrega.split('T')[0] 
                  : new Date(pedidoCompra.dataEntrega).toISOString().split('T')[0])
              : '',
            status: pedidoCompra.status || 'rascunho',
            frete: pedidoCompra.frete || '1',
            valorFrete: Number(pedidoCompra.valorFrete) || 0,
            despesas: Number(pedidoCompra.despesas) || 0,
            incluirFreteTotal: pedidoCompra.incluirFreteTotal ?? false,
            placaVeiculo: pedidoCompra.placaVeiculo || '',
            ufPlaca: pedidoCompra.ufPlaca || '',
            rntc: pedidoCompra.rntc || '',
            pesoLiquido: Number(pedidoCompra.pesoLiquido) || 0,
            pesoBruto: Number(pedidoCompra.pesoBruto) || 0,
            volume: Number(pedidoCompra.volume) || 0,
            especie: pedidoCompra.especie || '',
            marca: pedidoCompra.marca || '',
            numeracao: pedidoCompra.numeracao || '',
            quantidadeVolumes: Number(pedidoCompra.quantidadeVolumes) || 1,
            naturezaOperacao: pedidoCompra.naturezaOperacaoPadraoId || '',
            prazoPagamento: pedidoCompra.prazoPagamentoId || '',
            observacoes: pedidoCompra.observacoes || ''
          });

          // Preencher itens - mapear para o formato esperado pelo model.itens
          const itensMapped = (pedidoCompra.itens || []).map((it: any) => ({
            id: it.id,
            companyId: it.companyId || pedidoCompra.companyId,
            produtoId: it.produtoId || null,
            naturezaOperacaoId: it.naturezaOperacaoId || pedidoCompra.naturezaOperacaoPadraoId || '',
            codigo: it.codigo || '',
            nome: it.nome || '',
          ...
          if (pedidoCompra.fornecedorId) {
            // Tentar usar o objeto eager primeiro
            if ((pedidoCompra as any).fornecedor) {
              setFornecedorSelecionado((pedidoCompra as any).fornecedor);
            } else {
              // Fallback: buscar na lista de cadastros
              const c = cadastros.find((c: any) => c.id === pedidoCompra.fornecedorId);
              if (c) setFornecedorSelecionado(c);
            }
          }
          if (pedidoCompra.compradorId) {
            if ((pedidoCompra as any).comprador) {
              setCompradorSelecionado((pedidoCompra as any).comprador);
            } else {
              const v = cadastros.find((v: any) => v.id === pedidoCompra.compradorId);
              if (v) setCompradorSelecionado(v);
            }
          }
          if (pedidoCompra.transportadoraId) {
            if ((pedidoCompra as any).transportadora) {
              setTransportadoraSelecionada((pedidoCompra as any).transportadora);
            } else {
              const t = cadastros.find((t: any) => t.id === pedidoCompra.transportadoraId);
              if (t) setTransportadoraSelecionada(t);
            }
          }
          if (pedidoCompra.prazoPagamentoId) {
            if ((pedidoCompra as any).prazoPagamento) {
              setPrazoSelecionado((pedidoCompra as any).prazoPagamento);
            } else {
              const p = prazosPagamento.find((p: any) => p.id === pedidoCompra.prazoPagamentoId);
              if (p) setPrazoSelecionado(p);
            }
          }
          
          // Carregar forma de pagamento
          if (pedidoCompra.formaPagamentoId) {
            if ((pedidoCompra as any).formaPagamento) {
              setFormaPagamentoId(pedidoCompra.formaPagamentoId);
            } else {
              const f = formasPagamento.find((f: any) => f.id === pedidoCompra.formaPagamentoId);
              if (f) setFormaPagamentoId(f.id);
            }
          }
          
          // Carregar local de estoque
          if (pedidoCompra.localEstoqueId) {
            setLocalEstoqueId(pedidoCompra.localEstoqueId);
          }
        }
      } catch (err: any) {
        console.error('Erro ao carregar pedido de compra:', err);
        error('Erro', 'Falha ao carregar pedido de compra');
      }
    };
    loadInitialData();
  }, [token, id, activeCompanyId]);
  ...
  }, [token, model.itens, formData.naturezaOperacao, formData.valorFrete, formData.despesas, formData.incluirFreteTotal, fornecedorSelecionado]);
  ...
  async function salvar(confirmarEntrega: boolean = false) {
    ...
    if (!model.fornecedorId) { error('Erro', 'Selecione o fornecedor'); return; }
    ...
    // Validar campos obrigatórios para pedido de compra (sem motivoPerda)
    ...
    try {
      ...
      const modelSincronizado: PedidoCompra = {
        ...model,
        ...
        dataEmissao: formData.dataEmissao || (model.dataEmissao ? new Date(model.dataEmissao).toISOString().split('T')[0] : new Date().toISOString().split('T')[0]),
        ...
        dataEntrega: dataEntregaFormatada || undefined,
      ...
      const modelToSave: any = { 
        ...
        fornecedorId: modelSincronizado.fornecedorId,
        compradorId: modelSincronizado.compradorId || undefined,
      ...
      console.log('[Compras Page] Payload antes de salvar:', JSON.stringify(modelToSave, null, 2));
      console.log('[Compras Page] É novo pedido?', isNovo);
      
      let savedId = id;
      
      if (isNovo) {
        const saved = await criarPedidoCompra(modelToSave);
        savedId = saved.id;
        // Resetar dataLoaded para que os dados sejam carregados após o redirecionamento
        setDataLoaded(false);
        openSuccess({
          title: 'Pedido de Compra Salvo',
          message: 'Pedido de Compra criado com sucesso!',
          onClose: () => {
            router.replace(`/compras/${saved.id}`);
          }
        });
      } else {
        ...
        // Recarregar dados do pedido após atualização
        if (updated) {
          const pedidoAtualizado = await obterPedidoCompra(id, activeCompanyId || undefined);
          setModel(pedidoAtualizado);
          
          // Atualizar formData com dados atualizados
          setFormData(prev => ({
            ...prev,
            fornecedor: pedidoAtualizado.fornecedorId || prev.fornecedor,
            comprador: pedidoAtualizado.compradorId || prev.comprador,
            transportadora: pedidoAtualizado.transportadoraId || prev.transportadora,
            consumidorFinal: pedidoAtualizado.consumidorFinal ?? prev.consumidorFinal,
            indicadorPresenca: pedidoAtualizado.indicadorPresenca || prev.indicadorPresenca,
            formaPagamento: pedidoAtualizado.formaPagamentoId || prev.formaPagamento,
            parcelamento: pedidoAtualizado.parcelamento || prev.parcelamento,
            estoque: pedidoAtualizado.localEstoqueId || prev.estoque,
            pedido: pedidoAtualizado.numero || prev.pedido,
            nfe: pedidoAtualizado.serie || prev.nfe,
            numeroOrdemCompra: pedidoAtualizado.numeroOrdemCompra || prev.numeroOrdemCompra,
            dataPrevisao: pedidoAtualizado.dataPrevisaoEntrega 
              ? (typeof pedidoAtualizado.dataPrevisaoEntrega === 'string' 
                  ? pedidoAtualizado.dataPrevisaoEntrega.split('T')[0] 
                  : new Date(pedidoAtualizado.dataPrevisaoEntrega).toISOString().split('T')[0])
              : prev.dataPrevisao,
            dataEmissao: pedidoAtualizado.dataEmissao 
              ? (typeof pedidoAtualizado.dataEmissao === 'string' 
                  ? pedidoAtualizado.dataEmissao.split('T')[0] 
                  : new Date(pedidoAtualizado.dataEmissao).toISOString().split('T')[0])
              : prev.dataEmissao,
            dataEntrega: pedidoAtualizado.dataEntrega 
              ? (typeof pedidoAtualizado.dataEntrega === 'string' 
                  ? pedidoAtualizado.dataEntrega.split('T')[0] 
                  : new Date(pedidoAtualizado.dataEntrega).toISOString().split('T')[0])
              : prev.dataEntrega,
            status: pedidoAtualizado.status || prev.status,
            frete: pedidoAtualizado.frete || prev.frete,
            valorFrete: Number(pedidoAtualizado.valorFrete) || prev.valorFrete,
            despesas: Number(pedidoAtualizado.despesas) || prev.despesas,
            incluirFreteTotal: pedidoAtualizado.incluirFreteTotal ?? prev.incluirFreteTotal,
            placaVeiculo: pedidoAtualizado.placaVeiculo || prev.placaVeiculo,
            ufPlaca: pedidoAtualizado.ufPlaca || prev.ufPlaca,
            rntc: pedidoAtualizado.rntc || prev.rntc,
            pesoLiquido: Number(pedidoAtualizado.pesoLiquido) || prev.pesoLiquido,
            pesoBruto: Number(pedidoAtualizado.pesoBruto) || prev.pesoBruto,
            volume: Number(pedidoAtualizado.volume) || prev.volume,
            especie: pedidoAtualizado.especie || prev.especie,
            marca: pedidoAtualizado.marca || prev.marca,
            numeracao: pedidoAtualizado.numeracao || prev.numeracao,
            quantidadeVolumes: Number(pedidoAtualizado.quantidadeVolumes) || prev.quantidadeVolumes,
            naturezaOperacao: pedidoAtualizado.naturezaOperacaoPadraoId || prev.naturezaOperacao,
            prazoPagamento: pedidoAtualizado.prazoPagamentoId || prev.prazoPagamento,
            observacoes: pedidoAtualizado.observacoes || prev.observacoes
          }));
          
          // Atualizar itens
          const itensAtualizados = (pedidoAtualizado.itens || []).map((it: any) => ({
            id: it.id,
            companyId: it.companyId || pedidoAtualizado.companyId,
            produtoId: it.produtoId || null,
            naturezaOperacaoId: it.naturezaOperacaoId || pedidoAtualizado.naturezaOperacaoPadraoId || '',
            codigo: it.codigo || '',
            nome: it.nome || '',
            unidade: it.unidade || 'UN',
            quantidade: Number(it.quantidade) || 0,
  ...
  async function recalc() {
    if (!model.id) return;
    if (!activeCompanyId) {
      error('Erro', 'Empresa não selecionada');
      return;
    }
    try {
      await recalcularImpostos(model.id);
      const refreshed = await obterPedidoCompra(model.id, activeCompanyId);
      setModel(refreshed);
      ...
      <Layout>
        <div className="min-h-screen flex items-center justify-center bg-gray-50">
          <div className="flex flex-col items-center">
            ...
            <p className="text-purple-600 mt-4 font-medium">Carregando pedido de compra...</p>
          </div>
        </div>
      </Layout>
    );
  ...
  const blocked = model.status === 'faturado' || model.status === 'entregue' || pedidoCompraBloqueado;
  ...
  const handleInputChange = (field: string, value: any) => {
    ...
    if (field === 'fornecedor') {
      setModel(prev => ({ ...prev, fornecedorId: value }));
      const fornecedor = fornecedores.find((c: any) => c.id === value);
      if (fornecedor) setFornecedorSelecionado(fornecedor);
    } else if (field === 'comprador') {
      setModel(prev => ({ ...prev, compradorId: value }));
      const comprador = compradores.find((v: any) => v.id === value);
      if (comprador) setCompradorSelecionado(comprador);
      ...
      setModel(prev => ({ ...prev, dataPrevisaoEntrega: value ? new Date(value).toISOString().split('T')[0] : undefined }));
    } else if (field === 'dataEntrega') {
      const dataEntregaValue = value ? new Date(value).toISOString().split('T')[0] : undefined;
      ...
      // Campo NFe não pode ser editado pelo usuário, será preenchido automaticamente pela compra
  ...
  const handleSearchCadastros = (query: string) => {
    if (query.length >= 2) {
      ...
      setFornecedores(filtered.filter((c: any) => c.tiposCliente?.fornecedor));
      setCompradores(filtered.filter((c: any) => c.tiposCliente?.comprador));
      setTransportadoras(filtered.filter((c: any) => c.tiposCliente?.transportadora));
    }
  };

  // Função para editar fornecedor
  const handleEditFornecedor = async (fornecedorId: string) => {
    try {
      // Buscar os dados completos do fornecedor
      const fornecedor = await apiService.getCadastro(fornecedorId, token || '');
      ...
      if (fornecedor) {
        ...
        const queryParams = new URLSearchParams({
          ...
          returnUrl: `/pedidoCompras/${id || 'novo'}`
        });
        
        // Abrir em nova aba ou navegar
        router.push(`/cadastros/novo?${queryParams.toString()}`);
      } else {
        error('Erro', 'Não foi possível carregar os dados do fornecedor');
      }
    } catch (e: any) {
      console.error('Erro ao carregar dados do fornecedor:', e);
      error('Erro', e?.message || 'Erro ao carregar dados do fornecedor');
    }
  };

  // Função para editar comprador
  const handleEditComprador = (compradorId: string) => {
    // Implementar modal de edição de comprador
    console.log('Editar comprador:', compradorId);
  };
  ...
  const handleSelectProdutoModal = (produto: any) => {
    ...
    success('Produto selecionado', 'Dados carregados! Clique em "Adicionar Produto" para incluir no pedido de compra.');
  };
    ...
    <Layout>
      <div className="space-y-6">
        ...
        <HeaderCompra
          ...
          title={isNovo ? 'Novo Pedido de Compra' : `Pedido de Compra #${model.numero || 'N/A'}`}
          description={isNovo ? 'Crie um novo pedido de compra com produtos e configurações personalizadas' : 'Edite o pedido de compra com produtos e configurações personalizadas'}
          progressLabel="Progresso do Pedido de Compra"
        ...
        {/* Banner de Aviso - Pedido de Compra Concluído */}
        {blocked && (
          <motion.div
            initial={{ opacity: 0, y: -20 }}
            animate={{ opacity: 1, y: 0 }}
            ...
            <div className="flex items-center space-x-3">
              ...
              <div className="flex-1">
                <h3 className="text-lg font-semibold text-red-800">
                  Pedido de Compra Finalizado - Edição Bloqueada
                </h3>
                <p className="text-red-700 mt-1">
                  Este pedido de compra foi finalizado e não pode mais ser editado ou excluído.
                </p>
              </div>
            </div>
          </motion.div>
        ...
        {/* Configurações do Pedido de Compra */}
        <ConfiguracaoCompra
          formData={formData}
          ...
          showFornecedorDropdown={showFornecedorDropdown}
          setShowFornecedorDropdown={setShowFornecedorDropdown}
          showCompradorDropdown={showCompradorDropdown}
          setShowCompradorDropdown={setShowCompradorDropdown}
          naturezasOperacao={naturezasOperacao}
          isLoadingNaturezas={isLoadingNaturezas}
          fornecedorSelecionado={fornecedorSelecionado}
          setFornecedorSelecionado={setFornecedorSelecionado}
          compradorSelecionado={compradorSelecionado}
          setCompradorSelecionado={setCompradorSelecionado}
          onEditFornecedor={handleEditFornecedor}
          onEditComprador={handleEditComprador}
          ...
          title="Configurações do Pedido de Compra"
          description="Configure as informações básicas do pedido de compra"
        />
            ...
            <Card className="p-6 overflow-x-auto">
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
                ...
                <div className="bg-purple-50 p-4 rounded-lg border border-purple-200">
                  <p className="text-sm text-purple-600">Total do Pedido de Compra</p>
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <textarea
                    ...
                    className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-purple-500"
                    rows={4}
                    placeholder="Digite observações sobre o pedido de compra"
                  />
                </div>
              </div>
            </Card>
                ...
                <div className="flex items-center space-x-8">
                  ...
                  <div className="flex items-center space-x-6">
                    ...
                    <div className="text-center border-l-2 border-purple-200 pl-6">
                      <p className="text-xs text-purple-600 uppercase tracking-wide font-bold">Total do Pedido de Compra</p>
