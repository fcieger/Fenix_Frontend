...
import { createPartner, updatePartner, getPartner } from '@/services/partners-service';
    ...
    if (editParam === 'true' && idParam) {
      const loadPartnerData = async () => {
        try {
          ...
          // Se tem dataParam, tentar usar (pode estar desatualizado)
          // Mas preferir buscar do backend para garantir dados atualizados
          let partner: Partner;

          if (dataParam) {
            try {
              const parsedData = JSON.parse(dataParam);
              // Verificar se é um Partner válido do SDK
              if (parsedData.legalName && parsedData.id) {
                partner = parsedData as Partner;
              } else {
                // Se não é Partner válido, buscar do backend
                partner = await getPartner(idParam);
              }
            } catch {
              // Se falhar parse, buscar do backend
              partner = await getPartner(idParam);
            }
          } else {
            // Buscar do backend
          ...
          // Usar função helper para mapear Partner do SDK para formData
          const formDataMapped = mapSdkPartnerToFormData(partner);
          setFormData(formDataMapped);

          // Atualizar contador de contatos
          if (formDataMapped.contatos && formDataMapped.contatos.length > 1) {
            setContactCount(formDataMapped.contatos.length);
          }
        } catch (error) {
