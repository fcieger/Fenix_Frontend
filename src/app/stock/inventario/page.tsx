...
export default function InventarioPage() {
  ...
  const load = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const res = await fetch(`/api/stock/inventarios?companyId=${activeCompanyId}`, { cache: 'no-store' });
      const json = await res.json();
      if (json?.success) {
        setLista(json.data || []);
      }
  ...
  const criar = async () => {
    ...
    setLoading(true);
    try {
      const res = await fetch('/api/stock/inventarios', { 
        method: 'POST', 
  ...
  const abrir = (id: string) => {
    router.push(`/stock/inventario/${id}`);
  };
