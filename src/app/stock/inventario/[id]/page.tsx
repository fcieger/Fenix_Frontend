...
export default function InventarioEditarPage() {
  ...
  const salvar = async () => {
    ...
    try {
      ...
      const resContagens = await fetch(`/api/stock/inventarios/${inventario.id}/contagens`, {
        method: 'POST',
      ...
      const resReload = await fetch(`/api/stock/inventarios/${inventario.id}`, { cache: 'no-store' });
      ...
      <Layout>
        <div className="flex items-center justify-center min-h-screen">
          <div className="text-center">
            ...
            <Button onClick={() => router.push('/stock/inventario')} variant="outline">
