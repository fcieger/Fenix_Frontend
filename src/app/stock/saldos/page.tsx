...
export default function EstoqueSaldosPage() {
  ...
  const carregarSaldos = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/stock/saldos?${params.toString()}`, { cache: "no-store" });
      const json = await res.json();
      if (json?.success) {
        ...
        if (produtoSearch.trim()) {
          ...
          const produtosFiltrados = products.filter((p: Product) => {
            const displayProduct = mapProductToDisplay(p);
            return (
              (displayProduct.description && displayProduct.description.toLowerCase().includes(term)) ||
              (displayProduct.code && String(displayProduct.code).toLowerCase().includes(term))
            );
          }).map((p: Product) => p.id);
      ...
      const r = await fetch(`/api/stock/saldos/resumo?companyId=${activeCompanyId}&localId=${filtroLocal}`, { cache: "no-store" });
        ...
        try {
          ...
          if (json?.success) {
            ...
            if (precisaEnriquecer) {
              try {
                ...
                saldosData = saldosData.map((s) => {
                  ...
                  if (!prod) return s;
                  const displayProduct = mapProductToDisplay(prod);
                  return {
                    ...s,
                    produto_nome: s.produto_nome || displayProduct.description || s.produto_nome,
                    produto_sku: s.produto_sku || displayProduct.code || s.produto_sku,
                  };
                });
              } catch (e) {
            ...
            if (produtoSearch.trim()) {
              ...
              const produtos = response.data || [];
              const term = produtoSearch.toLowerCase();
              const produtosFiltrados = produtos.filter((p: Product) => {
                const displayProduct = mapProductToDisplay(p);
                return (
                  (displayProduct.description && displayProduct.description.toLowerCase().includes(term)) ||
                  (displayProduct.code && String(displayProduct.code).toLowerCase().includes(term))
