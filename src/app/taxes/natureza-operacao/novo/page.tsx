...
function NaturezaOperacaoForm() {
    ...
    try {
      if (token) {
        ...
        if (isEditMode) {
          ...
          openSuccess({ title: 'Atualizado com sucesso', message: 'Natureza de operação atualizada.', onClose: () => router.push('/taxes/natureza-operacao') });
        } else {
          console.log('🆕 Criando natureza com dados:', payloadToSave);
          await apiService.createNaturezaOperacao(payloadToSave as any, token);
          openSuccess({ title: 'Salvo com sucesso', message: 'Natureza de operação criada.', onClose: () => router.push('/taxes/natureza-operacao') });
        }
  ...
  const handleCancel = () => {
    router.push('/taxes/natureza-operacao');
  };
