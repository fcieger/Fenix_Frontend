...
export default function HistoricoVendasPage() {
  ...
  const [vendaParaCancelar, setVendaParaCancelar] = useState<Venda | null>(
    null
  );
    ...
    try {
      ...
      if (caixaResponse.ok) {
        ...
        if (
          caixaData.success &&
          caixaData.data &&
          caixaData.data.caixaAberto &&
          caixaData.data.caixa
        ) {
          ...
          showError(
            "Aviso",
            "Nenhum caixa aberto encontrado. Abra um caixa para ver o histórico."
          );
          setVendas([]);
          setLoading(false);
        }
        ...
        console.error(
          "❌ Erro ao buscar caixa:",
          caixaResponse.status,
          errorData
        );
  ...
  const carregarHistorico = async (caixaIdParam?: string) => {
    ...
    if (!token || !activeCompanyId || !idCaixa) {
      console.warn("⚠️ Parâmetros ausentes:", {
        token: !!token,
        activeCompanyId,
        idCaixa,
      });
    ...
    try {
      ...
      if (response.ok) {
        ...
        showError(
          "Erro",
          errorData.error || "Erro ao carregar histórico de vendas"
        );
        setVendas([]);
      }
    } catch (err) {
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    {vendasCanceladas > 0 && (
                      <span className="text-sm text-red-600 font-semibold">
                        {vendasCanceladas} cancelada
                        {vendasCanceladas !== 1 ? "s" : ""}
                      </span>
                    )}
                  </div>
                </div>
                                  ...
                                  <div className="mt-3 p-3 bg-red-50 border border-red-200 rounded-lg">
                                    ...
                                    {venda.dataCancelamento && (
                                      <p className="text-xs text-red-600 mt-1">
                                        Cancelada em:{" "}
                                        {formatDate(venda.dataCancelamento)}
                                      </p>
                                    )}
                                  </div>
                                )}
