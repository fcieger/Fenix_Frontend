...
export default function ConfiguracoesNFEPage() {
  ...
  const convertApiDataToNFeConfig = (
    data: ConfiguracaoNfeResponse[]
  ): NFeConfig[] => {
  ...
  const filteredConfiguracoes = configuracoes.filter((config) => {
    const matchesSearch =
      config.nome.toLowerCase().includes(searchQuery.toLowerCase()) ||
      config.descricao.toLowerCase().includes(searchQuery.toLowerCase()) ||
      config.empresa.toLowerCase().includes(searchQuery.toLowerCase());

    const matchesTipo =
      filterTipo === "ALL" || config.tipoModelo === filterTipo;
    const matchesAmbiente =
      filterAmbiente === "ALL" || config.ambiente === filterAmbiente;
            ...
            <div className="flex items-center space-x-4">
              ...
              <div>
                ...
                <p className="text-purple-100 mt-1 text-sm sm:text-base">
                  Gerencie as configurações de emissão de notas fiscais
                </p>
              </div>
            </div>
            <Button
                ...
                <p className="text-3xl font-bold text-yellow-900">
                  {
                    configuracoes.filter(
                      (c) => c.certificado.status === "vencido"
                    ).length
                  }
                </p>
              </div>
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-3xl font-bold text-purple-900">
                  {
                    configuracoes.filter((c) => c.ambiente === "producao")
                      .length
                  }
                </p>
              </div>
                ...
                {filteredConfiguracoes.map((config) => (
                  ...
                  >
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-sm space-y-1">
                        <div className="font-medium text-gray-900">
                          {config.certificado.nome}
                        </div>
                        <div className="text-gray-500">
                          Válido até: {config.certificado.validade}
                        </div>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                      {new Date(config.dataAtualizacao).toLocaleDateString(
                        "pt-BR",
                        {
                          day: "2-digit",
                          month: "2-digit",
