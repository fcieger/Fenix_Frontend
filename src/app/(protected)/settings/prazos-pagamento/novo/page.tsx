...
export default function NovoPrazoPagamentoPage() {
  ...
  const validateForm = () => {
    ...
    if (formData.tipo === "personalizado") {
      if (
        !formData.configuracoes.parcelas ||
        ...
        const totalPercentual = formData.configuracoes.parcelas.reduce(
      ...
      if (formData.tipo === "dias") {
        ...
        const total =
          (percentualEntrada || 0) +
          (percentualParcelas || 0) *
            (formData.configuracoes.numeroParcelas || 1);
        console.log("🔍 Percentuais (parcelas):", {
          percentualEntrada,
          percentualParcelas,
          numeroParcelas: formData.configuracoes.numeroParcelas,
          total,
        });
        if (total > 0 && Math.abs(total - 100) > 0.01) {
          newErrors.percentuais = "A soma dos percentuais deve ser 100%";
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setTimeout(() => {
        router.push("/settings/prazos-pagamento");
      }, 1500);
    } catch (e: any) {
  ...
  const handleTipoChange = (tipo: "dias" | "parcelas" | "personalizado") => {
    setFormData((prev) => ({
      ...
      configuracoes: {
        ...prev.configuracoes,
        parcelas:
          tipo === "personalizado" ? prev.configuracoes.parcelas || [] : [],
      },
    }));
  };
  ...
  const calcularVencimentos = () => {
    ...
    if (tipo === 'dias') {
      ...
      if (percentualRestante && percentualRestante > 0) {
        ...
        dataVencimento.setDate(dataVencimento.getDate() + (dias || 0));
      ...
      // Parcelas
      const valorParcela = (valorTotal * (percentualParcelas || 0)) / 100;
      for (let i = 0; i < (numeroParcelas || 0); i++) {
        const dataVencimento = new Date(dataVenda);
        dataVencimento.setDate(
          dataVencimento.getDate() + (i + 1) * (intervaloDias || 0)
        );
              ...
              >
                <Card className="p-8 bg-white/80 backdrop-blur-sm border-0 shadow-xl rounded-3xl">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    {(["dias", "parcelas", "personalizado"] as const).map(
                      (tipo, index) => {
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                        ...
                        <div className="space-y-2">
                          <Label
                            ...
                            className="text-sm font-semibold text-gray-700 flex items-center gap-2"
                          >
                            <Percent className="w-4 h-4" />% Entrada
                          </Label>
                          <Input
                        ...
                        <div className="space-y-2">
                          <Label
                            htmlFor="percentualRestante"
                            className="text-sm font-semibold text-gray-700 flex items-center gap-2"
                          >
                            <DollarSign className="w-4 h-4" />% Restante
                          </Label>
                          <Input
                  ...
                  >
                    <Card className="p-8 bg-white/80 backdrop-blur-sm border-0 shadow-xl rounded-3xl">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                          ...
                          <Input
                            ...
                            onChange={(e) =>
                              setFormData((prev) => ({
                                ...
                                configuracoes: {
                                  ...prev.configuracoes,
                                  numeroParcelas: parseInt(e.target.value) || 0,
                                },
                              }))
                            }
                          ...
                          <Input
                            ...
                            onChange={(e) =>
                              setFormData((prev) => ({
                                ...
                                configuracoes: {
                                  ...prev.configuracoes,
                                  intervaloDias: parseInt(e.target.value) || 0,
                                },
                              }))
                            }
                            className="h-12 rounded-xl border-2 border-gray-200 focus:border-blue-500 focus:ring-4 focus:ring-blue-100 transition-all duration-200"
                            ...
                            onChange={(e) =>
                              setFormData((prev) => ({
                                ...
                                configuracoes: {
                                  ...
                                  percentualParcelas:
                                    (100 - (parseInt(e.target.value) || 0)) /
                                    (prev.configuracoes.numeroParcelas || 1),
                                },
                              }))
                            }
                            className="h-12 rounded-xl border-2 border-gray-200 focus:border-blue-500 focus:ring-4 focus:ring-blue-100 transition-all duration-200"
                        ...
                        <div className="space-y-2">
                          <Label
                            htmlFor="percentualParcelas"
                            className="text-sm font-semibold text-gray-700 flex items-center gap-2"
                          >
                            <DollarSign className="w-4 h-4" />% por Parcela
                          </Label>
                          <Input
                      ...
                      <div className="space-y-4">
                        ...
                        {(formData.configuracoes.parcelas?.length ?? 0) === 0 && (
                          ...
                          >
                            ...
                            <p className="text-sm">
                              Clique em "Adicionar Parcela" para começar a
                              configurar
                            </p>
                          </motion.div>
                        )}
                  ...
                  <div className="bg-gradient-to-r from-amber-50 to-orange-50 rounded-2xl p-6">
                    ...
                    {calcularVencimentos().length > 0 ? (
                          ...
                          <div className="flex items-center justify-between">
                            <div className="flex items-center gap-3">
                              ...
                              <div>
                                ...
                                <p className="text-sm text-gray-600">
                                  {calcularVencimentos().length}{" "}
                                  {calcularVencimentos().length === 1
                                    ? "pagamento"
                                    : "pagamentos"}
                                </p>
                              </div>
                      ...
                      <div className="text-center py-8 text-gray-500">
                        ...
                        <p className="text-lg font-medium mb-2">
                          Configure o prazo para ver o teste
                        </p>
                        <p className="text-sm">
                          Preencha as configurações acima para visualizar o
                          cronograma
                        </p>
                      </div>
                    )}
                    ...
                    {formData.tipo === "dias" && (
                      <div className="p-4 bg-gradient-to-r from-orange-50 to-orange-100 rounded-xl">
                        ...
                        <p className="font-bold text-orange-700">
                          {formData.configuracoes.dias} dias
                        </p>
                      </div>
                    )}
                    {formData.tipo === "parcelas" && (
                      <div className="p-4 bg-gradient-to-r from-blue-50 to-blue-100 rounded-xl">
                        ...
                        <p className="font-bold text-blue-700">
                          {formData.configuracoes.numeroParcelas}x de{" "}
                          {formData.configuracoes.intervaloDias} dias
                        </p>
                      </div>
                    )}
                    {formData.tipo === "personalizado" && (
