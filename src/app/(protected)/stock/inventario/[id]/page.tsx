...
export default function InventarioEditarPage() {
  ...
  const load = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const res = await fetch(`/api/stock/inventarios/${id}`, {
        cache: "no-store",
      });
    ...
    setItens((prev) =>
      prev.map((item) => {
        if (item.id === itemId) {
          ...
          const diferenca =
            qtdContada !== null ? qtdContada - item.qtdSistema : 0;
  ...
  const salvar = async () => {
    ...
    try {
      ...
      const resContagens = await fetch(
        `/api/stock/inventarios/${inventario.id}/contagens`,
        {
          method: "POST",
      ...
      const itensRecarregados = jsonReload?.success
        ? jsonReload.data.itens || []
        : itens;
      ...
      for (const item of itensComDiferenca) {
        ...
        if (qtdSistema > 0) {
          const resSaida = await fetch("/api/stock/movimentos", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
              produtoId: item.produtoId,
        ...
        if (qtdContada > 0) {
          const resEntrada = await fetch("/api/stock/movimentos", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
              produtoId: item.produtoId,
    ...
    return (
      <div className="flex items-center justify-center min-h-screen">
        <div className="text-center">
          ...
          <Button
            onClick={() => router.push("/stock/inventario")}
            variant="outline"
          >
  ...
  const itensComDiferenca = itens.filter(
    (item) => Math.abs(item.diferenca) > 0.001
  );
  const itensComContagem = itens.filter((item) => {
    const qtd = item.qtdContada;
                  ...
                  >
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-sm font-medium text-gray-900">
                        {item.produto_nome || item.produtoId}
                      </div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-right">
                      <div className="text-sm text-gray-900 font-medium">
                        {Number(item.qtdSistema).toFixed(3)}
                      </div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
