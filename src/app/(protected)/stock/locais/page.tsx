...
export default function LocaisEstoquePage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/stock/locais?${params.toString()}`, {
        cache: "no-store",
      });
      ...
      if (json?.success) {
        ...
        openSuccess({
          title: "Erro",
          message: json.error || "Erro ao carregar locais de estoque",
        });
      }
    } catch (error: any) {
      ...
      openSuccess({
        title: "Erro",
        message: "Erro ao carregar locais: " + error.message,
      });
    } finally {
      setLoading(false);
    }
  }, [activeCompanyId, search]);
  ...
  const submit = async () => {
    ...
    try {
      ...
      if (!json?.success) {
        openSuccess({
          title: "Erro",
          message: json?.error || "Erro ao salvar",
        });
        return;
      }
      setFormOpen(false);
      ...
      openSuccess({
        title: "Erro",
        message: "Erro ao salvar local: " + error.message,
      });
    } finally {
      setSaving(false);
    }
  ...
  const remove = async (id: string) => {
    ...
    if (!json?.success) {
      openSuccess({
        title: "Erro",
        message: json?.error || "Erro ao excluir",
      });
      return;
    }
    load();
  ...
  const setAsDefault = async (localId: string) => {
    ...
    try {
      ...
      if (!json?.success) {
        openSuccess({
          title: "Erro",
          message: "Erro ao definir como padrão",
        });
      ...
      if (openSuccess)
        openSuccess({
          title: "Sucesso",
          message: "Local definido como padrão com sucesso!",
        });
    } catch (error: any) {
      openSuccess({
        title: "Erro",
          ...
          >
            <div className="mb-4 flex items-center justify-between">
              <h3 className="text-lg font-semibold text-gray-900">
                Configurações de Locais Padrão
              </h3>
              <Button
                variant="ghost"
                size="sm"
              ...
              <div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                  ...
                  <div className="flex items-end">
                    <Button
                      onClick={async () => {
                        if (!configCompanyId || !defaultCompanyLocalId)
                          return alert("Preencha companyId e local");
                        const res = await fetch(
                          "/api/stock/locais/default-company",
                          {
                            method: "POST",
                            headers: { "Content-Type": "application/json" },
                            body: JSON.stringify({
                              companyId: configCompanyId,
                              localId: defaultCompanyLocalId,
                            }),
                          }
                        );
                        const j = await res.json();
                        if (!j?.success)
                          return alert(j?.error || "Erro ao definir padrão");
              ...
              <div className="border-t border-gray-200 pt-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                  <div>
                    ...
                    <Input
                      placeholder="produtoId (UUID)"
                      value={produtoPadraoProdutoId}
                      onChange={(e) =>
                        setProdutoPadraoProdutoId(e.target.value)
                      }
                    />
                  </div>
                  <div>
                  ...
                  <div className="flex items-end">
                    <Button
                      onClick={async () => {
                        if (!produtoPadraoProdutoId || !produtoPadraoLocalId)
                          return alert("Preencha produtoId e local");
                        const res = await fetch(
                          "/api/stock/products/local-padrao",
                          {
                            method: "POST",
                            headers: { "Content-Type": "application/json" },
                            body: JSON.stringify({
                              produtoId: produtoPadraoProdutoId,
                              localId: produtoPadraoLocalId,
                            }),
                          }
                        );
                        const j = await res.json();
                        if (!j?.success)
                          return alert(
                            j?.error || "Erro ao definir padrão do produto"
                          );
                ...
                <div className="space-y-2">
                  ...
                  {!nome && (
                    <p className="text-xs text-gray-500 mt-1">
                      Digite um nome para identificar o local
                    </p>
                  )}
                </div>
