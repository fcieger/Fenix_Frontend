...
export default function EstoqueSaldosPage() {
  ...
  const carregarSaldos = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/stock/saldos?${params.toString()}`, {
        cache: "no-store",
      });
      const json = await res.json();
      if (json?.success) {
        ...
        if (produtoSearch.trim()) {
          const response = await listProducts({ search: produtoSearch.trim() });
        ...
        if (ordenacao === "nome") {
          ...
          saldosData = saldosData.sort(
            (a: SaldoRow, b: SaldoRow) => (b.qtd || 0) - (a.qtd || 0)
          );
        } else if (ordenacao === "qtd_asc") {
          saldosData = saldosData.sort(
            (a: SaldoRow, b: SaldoRow) => (a.qtd || 0) - (b.qtd || 0)
          );
        }
      ...
      const r = await fetch(
        `/api/stock/saldos/resumo?companyId=${activeCompanyId}&localId=${filtroLocal}`,
        { cache: "no-store" }
      );
        ...
        async () => {
          ...
          try {
            ...
            if (json?.success) {
              ...
              const precisaEnriquecer = saldosData.some(
                (s) => !s.produto_nome || !s.produto_sku
              );
              if (precisaEnriquecer) {
                try {
                  const response = await listProducts();
                  const produtos = response.data || [];
                  const mapIdToProduto = new Map<string, Product>();
                  produtos.forEach((p: Product) => mapIdToProduto.set(p.id, p));
                  saldosData = saldosData.map((s) => {
                    ...
                    return {
                      ...s,
                      produto_nome:
                        s.produto_nome || prod.description || s.produto_nome,
                      produto_sku: s.produto_sku || prod.code || s.produto_sku,
                    };
                  });
                } catch (e) {
                  console.warn(
                    "Não foi possível enriquecer dados do produto",
                    e
                  );
                }
              }
    ...
    <div className="space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-bold text-gray-900 mb-2">
              Itens e Saldos de Estoque
            </h1>
            <p className="text-gray-600">
              Consulte os saldos de produtos por local de estoque
            </p>
          </div>
            ...
            <div className="p-12 text-center">
              ...
              <p className="text-gray-500">
                {produtoSearch
                  ...
                  : "Não há produtos com saldo neste local."}
              </p>
            </div>
          ) : (
              ...
              <table className="min-w-full divide-y divide-gray-200">
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {saldos.map((row, idx) => {
                    ...
                    const isZero = qtd === 0;
                    const isAbaixoMinimo =
                      row.estoqueMinimo && qtd > 0 && qtd < row.estoqueMinimo;

                    return (
                      <tr
                        key={`${row.produtoId}-${row.localId}-${idx}`}
                        className="hover:bg-gray-50 transition-colors"
                      >
                        <td className="px-6 py-4 whitespace-nowrap">
                          <div className="text-sm font-medium text-gray-900">
                            {row.produto_nome || row.produtoId}
                          </div>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap">
      ...
      {!filtroLocal && (
        ...
        >
          ...
          <p className="text-gray-500">
            Escolha um local de estoque acima para visualizar os saldos dos
            produtos.
          </p>
        </motion.div>
      )}
