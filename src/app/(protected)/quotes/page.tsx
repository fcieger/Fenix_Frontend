...
// Helper function to map quote to display format
const mapQuoteToDisplay = (quote: Quote, partner?: Partner) => {
  return {
    number: quote.number || quote.id || "-",
    partnerName: partner?.legalName || "-",
    date: quote.createdAt || quote.date || new Date().toISOString(),
    validityDate:
      (quote as any).validityDate || (quote as any).dataValidade || null,
    total: (quote as any).total || (quote as any).totalGeral || 0,
  };
};
...
export default function OrcamentosPage() {
  ...
  const loadPartnerForQuote = async (quote: Quote): Promise<Partner | null> => {
    ...
    try {
      ...
      setPartnersCache((prev) =>
        new Map(prev).set(quote.partnerId!, partner as Partner)
      );
  ...
  useEffect(() => {
    ...
    const loadOrcamentos = async () => {
      try {
        ...
        const partnerPromises = quotes.map((quote) =>
          loadPartnerForQuote(quote)
        );
  ...
  const stats = {
    ...
    concluidos: orcamentos.filter((o) => o.status === QuoteStatus.APPROVED)
      .length,
                  ...
                  <div className="space-y-2 mb-4">
                    ...
                    {displayQuote.validityDate && (
                      <div className="flex items-center text-sm text-gray-600">
                        ...
                        Validade:{" "}
                        {new Date(displayQuote.validityDate).toLocaleDateString(
                          "pt-BR"
                        )}
                      </div>
                    )}
                    ...
                    <div className="flex gap-2">
                      ...
                      <motion.button
                        ...
                        onClick={() =>
                          handleDelete(
                            quote.id,
                            displayQuote.partnerName || "Orçamento"
                          )
                        }
            ...
            <table className="min-w-full divide-y divide-gray-200">
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {currentOrcamentos.map((quote, index) => {
                  ...
                  const partner = partnersCache.get(quote.partnerId || "");
                  const displayQuote = mapQuoteToDisplay(
                    quote,
                    partner || undefined
                  );
