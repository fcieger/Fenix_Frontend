...
export default function FormasPagamentoPage() {
  ...
  const fetchFormasPagamento = async () => {
    ...
    try {
      ...
      console.log(
        "🔍 Buscando formas de pagamento para company_id:",
        companyId
      );
  ...
  const handleDeleteForma = async (forma: FormaPagamento) => {
    if (
      confirm(
        `Tem certeza que deseja excluir a forma de pagamento "${forma.nome}"?`
      )
    ) {
      try {
  ...
  const handleSetPadrao = async (forma: FormaPagamento) => {
    try {
      const response = await fetch(
        `/api/formas-pagamento/${forma.id}/padrao?company_id=${companyId}`,
        {
          method: "PATCH",
        }
      );
      ...
      if (response.ok) {
        ...
        const result = await response.json();
        throw new Error(
          result.error || "Erro ao definir forma de pagamento como padrão"
        );
      }
    } catch (err) {
  ...
  const formasFiltradas = formasPagamento.filter(
    (forma) =>
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-slate-600 mt-1">
                Gerencie as formas de pagamento da empresa
              </p>
