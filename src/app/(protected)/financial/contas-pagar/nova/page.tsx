...
export default function NovaContaPagarPage() {
  ...
  const [cadastroSelecionado, setCadastroSelecionado] = useState<any | null>(
    null
  );
  ...
  const [contasContabeisFiltradas, setContasContabeisFiltradas] = useState<
    any[]
  >([]);
  const [showContaContabilDropdown, setShowContaContabilDropdown] =
    useState(false);
  const [contaContabilSelecionada, setContaContabilSelecionada] = useState<
    any | null
  >(null);
  const [isLoadingContasContabeis, setIsLoadingContasContabeis] =
    useState(false);
  ...
  const [showPrazoPagamentoDropdown, setShowPrazoPagamentoDropdown] =
    useState(false);
  ...
  const [formasPagamentoFiltradas, setFormasPagamentoFiltradas] = useState<
    any[]
  >([]);
  const [isLoadingFormasPagamento, setIsLoadingFormasPagamento] =
    useState(false);
  ...
  const [formasPagamentoMetodosFiltradas, setFormasPagamentoMetodosFiltradas] =
    useState<any[]>([]);
  const [isLoadingFormasPagamentoMetodos, setIsLoadingFormasPagamentoMetodos] =
    useState(false);

  // Estados para gerenciar contas bancárias
  const [contasBancarias, setContasBancarias] = useState<any[]>([]);
  const [contasBancariasFiltradas, setContasBancariasFiltradas] = useState<
    any[]
  >([]);
  const [isLoadingContasBancarias, setIsLoadingContasBancarias] =
    useState(false);
  ...
  const getApiBase = (): "/api/contas-pagar" | "/api/contas-receber" => {
    if (pathname && pathname.includes("/financial/contas-receber")) {
  ...
  const isReceber = !!(
    pathname && pathname.includes("/financial/contas-receber")
  );
  ...
  const [rateioDropdownItemId, setRateioDropdownItemId] = useState<
    string | null
  >(null);

  // Estados para rateio de centro de custo
  const [showRateioCentroCustoModal, setShowRateioCentroCustoModal] =
    useState(false);
  ...
  const [showRateioCentroCustoDropdown, setShowRateioCentroCustoDropdown] =
    useState(false);
  const [rateioCentroCustoDropdownItemId, setRateioCentroCustoDropdownItemId] =
    useState<string | null>(null);
  const [parcelasErrors, setParcelasErrors] = useState<
    Record<string, { comp?: boolean; forma?: boolean; conta?: boolean }>
  >({});
  ...
  const totais = {
    ...
    totalPago: contaPagar.parcelas.reduce(
      (sum, p) =>
        sum +
        (p.status === "pago" || p.status === "quitado" ? p.valorParcela : 0),
      0
    ),
    saldo: contaPagar.parcelas.reduce(
      (sum, p) =>
        sum +
  ...
  const filtrarCentrosCusto = (term: string) => {
    ...
    const filtered = centrosCusto.filter(
      (centro: any) =>
  ...
  const filtrarFormasPagamento = (term: string) => {
    ...
    const filtered = formasPagamento.filter(
      (prazo: any) =>
  ...
  const filtrarFormasPagamentoMetodos = (term: string) => {
    ...
    console.log(
      "📋 Total de formas disponíveis:",
      formasPagamentoMetodos.length
    );
    ...
    const filtered = formasPagamentoMetodos.filter(
      (forma: any) =>
    ...
    console.log(
      "🎯 Formas de pagamento filtradas encontradas:",
      filtered.length
    );
  ...
  const filtrarContasBancarias = (term: string) => {
    ...
    const filtered = contasBancarias.filter(
      (conta: any) =>
        conta.descricao?.toLowerCase().includes(term.toLowerCase()) ||
  ...
  const selecionarPrazoPagamento = (prazo: any) => {
    ...
    const parcelasComTitulos = novasParcelas.map((parcela, index) => ({
      ...parcela,
      tituloParcela: gerarTituloParcela(
        contaPagar.titulo,
        index + 1,
        novasParcelas.length
      ),
    }));
  ...
  const validateParcelaPago = (parcela: Parcela) => {
    ...
    setParcelasErrors((prev) => ({
      ...prev,
      [parcela.id]: hasAny ? missing : {},
    }));
    return !hasAny;
  };
  ...
  const pagarParcelaSePersistida = async (parcela: Parcela) => {
    try {
      ...
      const body = {
        ...
        data_pagamento:
          parcela.dataPagamento || new Date().toISOString().slice(0, 10),
      ...
      const res = await fetch(
        `${getApiBase()}/parcelas/${parcela.id}/${getParcelasAction("pagar")}`,
        {
          method: "POST",
      ...
      if (json.data?.titulo) {
        ...
        setContaPagar((prev) => ({
          ...prev,
          status: statusNormalizado as
            | "PENDENTE"
            | "PAGO"
            | "PARCIAL"
            | "CANCELADO",
          dataQuitacao: tituloAtualizado.data_quitacao
            ? String(tituloAtualizado.data_quitacao).split("T")[0]
            : prev.dataQuitacao,
        }));
      ...
      if (json.data?.parcela) {
        setContaPagar((prev) => ({
          ...
          parcelas: prev.parcelas.map((p) =>
            p.id === parcela.id ? { ...p, status: "pago" as const } : p
          ),
        }));
      }
    } catch (e) {
  ...
  const converterStringParaNumeroComNegativo = (
    valorString: string
  ): number => {
  ...
  const gerarParcelas = (
    ...
    // Se é prazo parcelado
      ...
      prazoSelecionado.configuracoes?.parcelas &&
      prazoSelecionado.configuracoes.parcelas.length > 0
    ) {
  ...
  const loadContasContabeis = async () => {
    ...
    try {
      ...
      if (!companyId) {
        console.log(
          "⚠️ Company ID não disponível para buscar contas contábeis"
        );
        return;
      }
      const response = await fetch(
        `/api/contas-contabeis?company_id=${companyId}`
      );
  ...
  const loadCentrosCusto = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `/api/centros-custos?company_id=${companyId}`
      );
      const data = await response.json();
  ...
  const loadPrazosPagamento = async () => {
    ...
    try {
      ...
      if (!companyId) {
        console.log(
          "⚠️ Company ID não disponível para buscar prazos de pagamento"
        );
        return;
      }

      const response = await fetch(
        `/api/prazos-pagamento?company_id=${companyId}`
      );
      const data = await response.json();
      ...
      if (response.ok && data.success) {
        ...
        console.log(
          "✅ Prazos de pagamento carregados:",
          prazos.length,
          "registros"
        );
        console.log("📋 Dados dos prazos:", prazos);
        // Ordenar alfabeticamente por nome
        const prazosOrdenados = prazos.sort((a: any, b: any) =>
  ...
  const loadFormasPagamentoMetodos = async () => {
    ...
    try {
      ...
      if (!companyId) {
        console.log(
          "⚠️ Company ID não disponível para buscar formas de pagamento"
        );
        return;
      }

      const response = await fetch(
        `/api/formas-pagamento?company_id=${companyId}`
      );
      const data = await response.json();
      ...
      if (response.ok && data.success) {
        ...
        // Ordenar alfabeticamente por nome
        const formasOrdenadas = formas.sort((a: any, b: any) =>
          a.nome.localeCompare(b.nome)
        );
  ...
  const loadContasBancarias = async () => {
    ...
    try {
      ...
      if (!companyId) {
        console.log(
          "⚠️ Company ID não disponível para buscar contas bancárias"
        );
        return;
      }
      ...
      if (response.ok && data.success) {
        const contas = data.data || [];
        console.log(
          "✅ Contas bancárias carregadas:",
          contas.length,
          "registros"
        );
        // Ordenar alfabeticamente por descrição
        const contasOrdenadas = contas.sort((a: any, b: any) =>
  ...
  useEffect(() => {
    ...
    const contasFiltradas = contasBancarias.filter(
      (conta: any) => conta.tipo_conta?.toUpperCase() !== "INVESTIMENTO"
    );
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      ...
      if (
        !target.closest(".forma-pagamento-dropdown") &&
        !target.closest('[class*="forma-pagamento-dropdown-"]')
      ) {
        const dropdowns = document.querySelectorAll(
          '[class*="forma-pagamento-dropdown-"]'
        );
      ...
      if (
        !target.closest(".conta-corrente-dropdown") &&
        !target.closest('[class*="conta-corrente-dropdown-"]')
      ) {
        const dropdowns = document.querySelectorAll(
          '[class*="conta-corrente-dropdown-"]'
        );
        dropdowns.forEach((dropdown) => {
          dropdown.classList.add("hidden");
        });
    ...
    const novosItems = rateioItems.map((item) => {
      if (item.id === id) {
        ...
        if (campo === "percentual" && contaPagar.valorTotal > 0) {
          ...
          itemAtualizado.percentual =
            Math.round(percentualCalculado * 100) / 100; // Arredondar para 2 casas decimais
        }
  ...
  const selecionarContaContabilRateio = (
    id: string,
    nome: string,
    itemId: string
  ) => {
  ...
  const salvarRateio = () => {
    ...
    if (diferenca > 0) {
      alert(
        `O total do rateio (R$ ${rateioTotal.toFixed(
          2
        )}) deve ser exatamente igual ao valor total do título (R$ ${contaPagar.valorTotal.toFixed(
          2
        )}). Diferença atual: R$ ${diferenca.toFixed(2)}`
      );
      return;
    }
    ...
    const itensSemContaContabil = rateioItems.filter(
      (item) => !item.contaContabilId || !item.contaContabilNome
    );
    if (itensSemContaContabil.length > 0) {
      alert(
        "Todos os itens de rateio devem ter uma conta contábil selecionada"
      );
      return;
    }

    // Validar se todos os valores são maiores que zero
    const itensComValorZero = rateioItems.filter(
      (item) => !item.valor || item.valor <= 0
    );
  ...
  const removerItemRateioCentroCusto = (id: string) => {
    setRateioCentroCustoItems(
      rateioCentroCustoItems.filter((item) => item.id !== id)
    );
  };

  const atualizarItemRateioCentroCusto = (
    id: string,
    campo: string,
    valor: any
  ) => {
  ...
  const selecionarCentroCustoRateio = (
    id: string,
    nome: string,
    itemId: string
  ) => {
  ...
  const salvarRateioCentroCusto = () => {
    ...
    if (diferenca > 0) {
      alert(
        `O total do rateio (R$ ${rateioCentroCustoTotal.toFixed(
          2
        )}) deve ser exatamente igual ao valor total do título (R$ ${contaPagar.valorTotal.toFixed(
          2
        )}). Diferença atual: R$ ${diferenca.toFixed(2)}`
    ...
    const itensSemCentroCusto = rateioCentroCustoItems.filter(
      (item) => !item.centroCustoId || !item.centroCustoNome
    );
    if (itensSemCentroCusto.length > 0) {
      alert(
        "Todos os itens de rateio devem ter um centro de custo selecionado"
      );
      return;
    }

    // Validar se todos os valores são maiores que zero
    const itensComValorZero = rateioCentroCustoItems.filter(
      (item) => !item.valor || item.valor <= 0
    );
    if (itensComValorZero.length > 0) {
      alert("Todos os itens de rateio devem ter um valor maior que zero");
    ...
    console.log(
      "Rateio centro custo aplicado ao título:",
      rateioCentroCustoItems
    );
  ...
  const handleSalvar = async () => {
    ...
    try {
      ...
      const novasErros: Record<
        string,
        { comp?: boolean; forma?: boolean; conta?: boolean }
      > = {};
      ...
      console.log("[NovaContaPagar] Salvando:", {
        method,
        endpoint,
        idToUse,
        urlId,
      ...
      if (data.success) {
        ...
        if (method === "POST" && data.data?.id) {
          ...
          setUrlId(newId);
          console.log(
            "[NovaContaPagar] Título criado, atualizando para modo edição:",
            { id: newId }
          );
        }
        openSuccess({
          ...
          message:
            getApiBase() === "/api/contas-receber"
              ? "A conta a receber foi salva com êxito."
              : "A conta a pagar foi salva com êxito.",
        });
      } else {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        console.log("[NovaContaPagar] Resposta HTTP", {
          status: res.status,
          ok: res.ok,
          url: res.url,
        });
        ...
        console.log("[NovaContaPagar] Dados do título", {
          ...
          parcelas_count: Array.isArray(data?.parcelas)
        ...
        const normalizeStatus = (s: any) => {
          ...
          if (up === "PAGO" || up === "PARCIAL" || up === "CANCELADO")
            return up;
          return "PENDENTE";
        };
        const novoTitulo = {
          ...
          dataEmissao: data.data_emissao
            ? String(data.data_emissao).split("T")[0]
            : new Date().toISOString().split("T")[0],
          dataQuitacao: data.data_quitacao
            ? String(data.data_quitacao).split("T")[0]
            : "",
          competencia: data.competencia || "",
          centroCusto: data.centro_custo_id || "",
          origem: data.origem || "",
          observacoes: data.observacoes || "",
          status: normalizeStatus(data.status) as
            | "PENDENTE"
            | "PAGO"
            | "PARCIAL"
            | "CANCELADO",
          parcelas: (data.parcelas || []).map((p: any) => ({
            ...
            dataPagamento: p.data_pagamento
              ...
              : "",
            dataCompensacao: p.data_compensacao
              ? String(p.data_compensacao).split("T")[0]
              : "",
        ...
        if (Array.isArray((data as any).rateioContaContabil)) {
          const itens = (data as any).rateioContaContabil.map((r: any) => ({
            ...
            contaContabilNome:
              r.conta_contabil_descricao || r.contaContabilNome || "",
            valor: Number(r.valor || 0),
            percentual: Number(r.percentual || 0),
          }));
          setRateioItems(itens);
          const total = itens.reduce(
            (sum: number, it: any) => sum + (Number(it.valor) || 0),
            0
          );
        ...
        if (Array.isArray((data as any).rateioCentroCusto)) {
          const itensCC = (data as any).rateioCentroCusto.map((r: any) => ({
            ...
            centroCustoNome:
              r.centro_custo_descricao || r.centroCustoNome || "",
            valor: Number(r.valor || 0),
            percentual: Number(r.percentual || 0),
          }));
          setRateioCentroCustoItems(itensCC);
          const totalCC = itensCC.reduce(
            (sum: number, it: any) => sum + (Number(it.valor) || 0),
            0
          );
  ...
  useEffect(() => {
    if (cadastroSelecionado?.id && cadastros?.length) {
      ...
      if (
        found &&
        !cadastroSelecionado.nomeRazaoSocial &&
        !cadastroSelecionado.nomeFantasia
      ) {
  ...
  useEffect(() => {
    if (contaPagar?.contaContabil && contasContabeis?.length) {
      const found = contasContabeis.find(
        (cc: any) => cc.id === contaPagar.contaContabil
      );
  ...
  useEffect(() => {
    if (contaPagar?.centroCusto && centrosCusto?.length) {
      const found = centrosCusto.find(
        (cc: any) => cc.id === contaPagar.centroCusto
      );
  ...
  useEffect(() => {
    if (prazoSelecionado?.id && formasPagamento?.length) {
      const found = formasPagamento.find(
        (p: any) => p.id === prazoSelecionado.id
      );
    ...
    if (contaPagar.parcelas.length > 0 && formasPagamentoMetodos.length > 0) {
      const parcelasAtualizadas = contaPagar.parcelas.map((parcela) => {
        if (parcela.formaPagamentoId && !parcela.formaPagamentoNome) {
          const forma = formasPagamentoMetodos.find(
            (f: any) => f.id === parcela.formaPagamentoId
          );
          if (forma) {
      ...
      const temMudancas = parcelasAtualizadas.some(
        (p, i) =>
      ...
      const parcelasAtualizadas = contaPagar.parcelas.map((parcela) => {
        if (parcela.contaCorrenteId && !parcela.contaCorrenteNome) {
          const conta = contasBancarias.find(
            (c: any) => c.id === parcela.contaCorrenteId
          );
          if (conta) {
        ...
        >
          <div className="max-w-7xl mx-auto">
            <div className="flex items-center justify-between">
                ...
                <div className="flex items-center space-x-4">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-purple-100 mt-1 text-lg">
                      {isReceber
                        ? "Cadastre um novo recebimento com parcelas e configurações"
              ...
              <div className="flex items-center space-x-4">
                <div className="flex items-center space-x-3 text-white"></div>
                ...
                <div className="flex items-center space-x-3">
                  ...
                  <Button
                    ...
                    title={
                      contaPagar.status === "PAGO"
                        ? "Título quitado não pode ser editado"
                        : ""
                    }
                  >
                    {loading ? (
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-gray-600 mt-1">
                    {isReceber
                      ? "Configure as informações básicas do Contas a Receber"
                      : "Configure as informações básicas da conta a pagar"}
                  </p>
                </div>
              </div>
            </div>
              ...
              <div className="space-y-8">
                ...
                <div className="grid grid-cols-1 xl:grid-cols-3 gap-8">
                  ...
                  <div className="space-y-6">
                    ...
                    >
                      <Label
                        htmlFor="cadastro"
                        className="text-sm font-semibold text-gray-700 flex items-center gap-2 mb-3"
                      >
                        ...
                        <Input
                          ...
                          value={
                            rateioItems.length > 0
                              ? `${rateioItems.length} conta(s) configurada(s)`
                              : contaContabilSelecionada
                              ? contaContabilSelecionada.descricao
                        ...
                        <Input
                          ...
                          value={
                            rateioCentroCustoItems.length > 0
                              ? `${rateioCentroCustoItems.length} centro(s) configurado(s)`
                              : centroCustoSelecionado
                              ? centroCustoSelecionado.descricao
                              ...
                              {isLoadingFormasPagamento ? (
                                ...
                                formasPagamentoFiltradas.map((prazo: any) => {
                                  ...
                                  if (prazo.configuracoes?.dias !== undefined) {
                                    ...
                                    diasTexto =
                                      dias > 0 ? `${dias} dias` : "À vista";
                                  } else if (
                                    prazo.configuracoes?.parcelas &&
                                    prazo.configuracoes.parcelas.length > 0
                                  ) {
                                    // Para prazos com parcelas, mostrar o maior prazo
                                    const maxDias = Math.max(
                                      ...prazo.configuracoes.parcelas.map(
                                        (p: any) => p.dias
                                      )
                                    );
                    ...
                    >
                      ...
                      <div className="relative">
                        <Input
                          ...
                          onChange={(e) =>
                            handleInputChange("dataQuitacao", e.target.value)
                          }
                      ...
                      <div className="relative">
                        <Input
                          ...
                          value={(() => {
                            ...
                            if (
                              contaPagar.competencia &&
                              contaPagar.competencia.includes("/")
                            ) {
                              const [mes, ano] =
                                contaPagar.competencia.split("/");
                ...
                >
                  <div className="mb-6">
                    ...
                    <p className="text-gray-600 mt-1">
                      {isReceber
                        ? "Configure as parcelas do recebimento"
                                ...
                                <div className="flex flex-col items-center gap-4">
                                  ...
                                  <div>
                                    ...
                                    <p className="text-gray-500 mb-4">
                                      Configure o prazo de pagamento ou adicione
                                      parcelas manualmente
                                    </p>
                                    <Button
                            ...
                            contaPagar.parcelas.map((parcela, index) => (
                              ...
                              >
                                <td className="px-3 py-2">
                                  <Input
                                    ...
                                    onChange={(e) => {
                                      ...
                                      novasParcelas[index].tituloParcela =
                                        e.target.value;
                                      setContaPagar((prev) => ({
                                        ...prev,
                                ...
                                <td className="px-3 py-2">
                                  <Input
                                    ...
                                    onChange={(e) => {
                                      ...
                                      novasParcelas[index].dataVencimento =
                                        e.target.value;
                                      setContaPagar((prev) => ({
                                        ...prev,
                                        parcelas: novasParcelas,
                                ...
                                <td className="px-3 py-2">
                                  <Input
                                    ...
                                    onChange={(e) => {
                                      ...
                                      novasParcelas[index].dataCompensacao =
                                        e.target.value;
                                      setContaPagar((prev) => ({
                                        ...prev,
                                        parcelas: novasParcelas,
                                    ...
                                    className={`border rounded px-2 py-1 text-xs text-gray-700 focus:ring-1 transition-all duration-200 bg-white w-full ${
                                  ...
                                  {parcelasErrors[parcela.id]?.comp && (
                                    <div className="text-[10px] text-red-600 mt-1">
                                      Obrigatório quando status = PAGO
                                    </div>
                                  )}
                                </td>
                                <td className="px-3 py-2">
                                  <Input
                                    ...
                                    onChange={(e) => {
                                      filtrarFormasPagamentoMetodos(
                                        e.target.value
                                      );
                                    }}
                                    onFocus={() => {
                                      const dropdown = document.querySelector(
                                        `.forma-pagamento-dropdown-${index}`
                                      );
                                      if (dropdown) {
                                  ...
                                  <div
                                    className={`forma-pagamento-dropdown-${index} absolute z-50 w-80 mt-1 bg-white border border-gray-200 rounded-lg shadow-xl max-h-48 overflow-y-auto hidden`}
                                  >
                                    ...
                                    ) : formasPagamentoMetodosFiltradas.length >
                                      0 ? (
                                      formasPagamentoMetodosFiltradas.map(
                                        (forma: any) => (
                                          <div
                                            key={forma.id}
                                ...
                                <td className="px-3 py-2">
                                  ...
                                  <div
                                    className={`conta-corrente-dropdown-${index} absolute z-50 w-80 mt-1 bg-white border border-gray-200 rounded-lg shadow-xl max-h-48 overflow-y-auto hidden`}
                                  >
                                ...
                                <td className="px-3 py-2">
                                  <Input
                                    ...
                                    onChange={(e) => {
                                      const valor = converterStringParaNumero(
                                        e.target.value
                                      );
                                      const novasParcelas = [
                                        ...contaPagar.parcelas,
                                      ...
                                      novasParcelas[index].valorTotal =
                                        valor + novasParcelas[index].diferenca;
                                      setContaPagar((prev) => ({
                                        ...prev,
                                        parcelas: novasParcelas,
                                      }));
                                ...
                                <td className="px-3 py-2">
                                  <Input
                                    ...
                                    onChange={(e) => {
                                      const valor =
                                        converterStringParaNumeroComNegativo(
                                          e.target.value
                                        );
                                      const novasParcelas = [
                                        ...contaPagar.parcelas,
                                      ...
                                      novasParcelas[index].valorTotal =
                                        novasParcelas[index].valorParcela +
                                        valor;
                                      setContaPagar((prev) => ({
                                        ...prev,
                                        parcelas: novasParcelas,
                                  ...
                                  <Select
                                    ...
                                    onValueChange={async (
                                      ...
                                      if (
                                        statusAnterior === "pago" &&
                                        value === "pendente" &&
                                        isUuid(novasParcelas[index].id || "")
                                      ) {
                                        const confirmou = await openConfirm({
                                          title:
                                            "Desfazer pagamento da parcela?",
                                          message:
                                            "Essa ação vai reverter o lançamento na movimentação bancária e marcar a parcela como pendente. Deseja continuar?",
                                        ...
                                        try {
                                          const res = await fetch(
                                            `${getApiBase()}/parcelas/${
                                              novasParcelas[index].id
                                            }/${getParcelasAction("estornar")}`,
                                            { method: "POST" }
                                          );
                                          const json = await res.json();
                                          if (!res.ok || !json.success) {
                                            console.error(
                                              "Falha ao estornar pagamento da parcela:",
                                              json?.error
                                            );
                                            return;
                                          }
                                          ...
                                          setContaPagar((prev) => ({
                                            ...
                                            status: ((
                                              ...
                                              | "PARCIAL"
                                              | "CANCELADO",
                                            dataQuitacao: json.data?.titulo
                                              ?.data_quitacao
                                              ? String(
                                                  json.data.titulo.data_quitacao
                                                ).split("T")[0]
                                              : "",
                                          }));
                                        } catch (e) {
                                          console.error(
                                            "Erro ao estornar parcela:",
                                            e
                                          );
                                          return;
                                        }
              ...
              <div className="flex items-center space-x-8">
                <div className="flex items-center space-x-2 text-gray-600">
                  ...
                  <span className="text-sm font-medium">
                    {contaPagar.parcelas.length} parcela
                    {contaPagar.parcelas.length !== 1 ? "s" : ""}
                  </span>
                </div>
                ...
                <div className="flex items-center space-x-8">
                  <div className="text-center">
                    ...
                    <div className="text-xl font-bold text-purple-600">
                      R${" "}
                      {formatCurrency(
                        contaPagar.parcelas.reduce(
                          (sum, parcela) => sum + parcela.valorTotal,
                          0
                        )
                      )}
                    </div>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h2 className="text-2xl font-bold">
                      Rateio de Conta Contábil
                    </h2>
                    <p className="text-purple-100 text-sm">
                      Configure a distribuição entre múltiplas contas
                    </p>
                  </div>
                </div>
                <button
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  ...
                  <div className="space-y-2">
                    ...
                    <p className="font-bold text-gray-800 text-xl">
                      R$ {contaPagar.valorTotal.toFixed(2)}
                    </p>
                  </div>
                  <div className="space-y-2">
                    <div className="flex items-center gap-2 text-sm text-gray-600">
              ...
              >
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  ...
                  <div className="space-y-2">
                    ...
                    >
                      {rateioTotal > contaPagar.valorTotal ? "+" : ""}R${" "}
                      {(rateioTotal - contaPagar.valorTotal).toFixed(2)}
                    </p>
                    <div
                  ...
                  <div className="mt-4 p-3 bg-red-100 border border-red-200 rounded-xl">
                    <div className="flex items-center gap-2 text-red-700">
                      ...
                      <span className="font-medium">
                        O total do rateio deve ser exatamente igual ao valor
                        total do título
                      </span>
                    </div>
                  </div>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-orange-100 text-sm">
                      Configure a distribuição entre múltiplos centros
                    </p>
                  </div>
                </div>
                <button
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                      ...
                      <div className="space-y-3">
                        ...
                        <div className="relative rateio-centro-custo-dropdown">
                          ...
                          {/* Dropdown de Centros de Custo */}
                          {showRateioCentroCustoDropdown &&
                            rateioCentroCustoDropdownItemId === item.id &&
                            centrosCustoFiltrados.length > 0 && (
                              <div className="absolute top-full left-0 right-0 bg-white border border-gray-200 rounded-xl shadow-xl z-20 max-h-48 overflow-y-auto mt-1">
                                {centrosCustoFiltrados.map((centro: any) => (
                                  <div
                      ...
                      <div className="space-y-3">
                        ...
                        <div className="relative">
                          <Input
                            ...
                            value={item.valor || ""}
                            onChange={(e) =>
                              atualizarItemRateioCentroCusto(
                                item.id,
                                "valor",
                                parseFloat(e.target.value) || 0
                              )
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  <div className="text-center">
                    ...
                    <div className="text-2xl font-bold text-orange-700">
                      R$ {rateioCentroCustoTotal.toFixed(2)}
                    </div>
                  </div>
                  <div className="text-center">
                    <div className="text-sm text-gray-600 mb-2">
                  ...
                  <div className="text-center">
                    ...
                    >
                      R${" "}
                      {(rateioCentroCustoTotal - contaPagar.valorTotal).toFixed(
                        2
                      )}
                    </div>
                  </div>
