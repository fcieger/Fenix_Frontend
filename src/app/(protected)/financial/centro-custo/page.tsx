...
export default function CentroCustosPage() {
  ...
  const [centroEditando, setCentroEditando] = useState<CentroCusto | null>(
    null
  );
    ...
    if (
      window.confirm(
        `Tem certeza que deseja ${
          centro.ativo ? "inativar" : "excluir"
        } o centro de custo "${centro.descricao}"?`
      )
    ) {
      try {
  ...
  const handleSaveCentro = async (
    data: CreateCostCenterRequest | UpdateCostCenterRequest
  ) => {
    try {
      if (centroEditando) {
        // Editar
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-slate-600 mt-1">
                Gerencie seus centros de custos
              </p>
