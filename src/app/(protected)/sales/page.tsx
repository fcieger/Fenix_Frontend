...
import { translateOrderStatus } from "@/lib/sdk/enum-translators";

// Helper function to map sales order to display format
const mapSalesOrderToDisplay = (order: SalesOrder, partner?: Partner) => {
  return {
...
export default function PedidosVendaPage() {
  ...
  const { user, token, isAuthenticated, isLoading, activeCompanyId } =
    useAuth();
  ...
  const [currentPdfPedido, setCurrentPdfPedido] = useState<{
    id: string;
    numero: string;
  } | null>(null);
  ...
  const filteredPedidoVendas = pedidoVendas.filter((pv) => {
    const partner = partnersCache.get(pv.partnerId || "");
    const matchesSearch =
      partner?.legalName?.toLowerCase().includes(searchTerm.toLowerCase()) ||
      pv.number?.includes(searchTerm) ||
      (pv as any).seller?.name
        ?.toLowerCase()
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      setError(
        error?.message || "Erro ao excluir pedido de venda. Tente novamente."
      );
    }
  };
  ...
  const stats = {
    ...
    finalizados: pedidoVendas.filter((o) => o.status === OrderStatus.COMPLETED)
      .length,
    pendentes: pedidoVendas.filter((o) => o.status === OrderStatus.IN_PROGRESS)
      .length,
    rascunhos: pedidoVendas.filter((o) => o.status === OrderStatus.DRAFT)
      .length,
      ...
      {isLoadingPedidoVendas ? (
        ...
        >
          {currentPedidoVendas.map((order, index) => {
            ...
            const displayOrder = mapSalesOrderToDisplay(
              order,
              partner || undefined
            );
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex gap-2">
                      ...
                      <motion.button
                        ...
                        whileTap={{ scale: 0.95 }}
                        onClick={() =>
                          handleExportarPDF(order.id, displayOrder.number)
                        }
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                                  <div className="bg-white p-4 rounded-lg shadow-sm">
                                    ...
                                    <p className="text-sm text-gray-900">
                                      {(() => {
                                        const partner = detalhes.partnerId
                                          ? partnersCache.get(
                                              detalhes.partnerId
                                            )
                                          : null;
                                    ...
                                    {(() => {
                                      const partner = detalhes.partnerId
                                        ? partnersCache.get(detalhes.partnerId)
                                        : null;
        ...
        >
          <div className="flex items-center justify-between">
            <div className="text-sm text-gray-700">
              Mostrando {startIndex + 1} a {endIndex} de {totalPedidoVendas}{" "}
              resultados
            </div>
            ...
            <div className="flex items-center gap-2">
              ...
              <div className="flex gap-1">
                {Array.from(
                  { length: Math.ceil(totalPedidoVendas / itemsPerPage) },
                  (_, i) => i + 1
                )
                  .filter((page) => {
              ...
              <Button
                ...
                onClick={() =>
                  setCurrentPage((prev) =>
                    Math.min(
                      prev + 1,
                      Math.ceil(totalPedidoVendas / itemsPerPage)
                    )
                  )
                }
                disabled={
                  currentPage === Math.ceil(totalPedidoVendas / itemsPerPage)
                }
              >
