...
export default function PedidosCompraPage() {
  ...
  const [expandedPedidoCompra, setExpandedPedidoCompra] = useState<
    string | null
  >(null);
  ...
  const loadPartnerForOrder = async (
    order: PurchaseOrder
  ...
  useEffect(() => {
    const loadPedidosCompra = async () => {
      try {
        ...
        const partnerPromises = orders.map((order) =>
          loadPartnerForOrder(order)
        );
  ...
  const currentPedidosCompra = filteredPedidosCompra.slice(
    startIndex,
    endIndex
  );
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      setError(
        error?.message || "Erro ao excluir pedido de compra. Tente novamente."
      );
    }
  };
    ...
    if (expandedPedidoCompra === id) {
      ...
      if (!pedidoCompraDetalhes[id]) {
        try {
          ...
          console.error(
            "Erro ao carregar detalhes do pedido de compra:",
            error
          );
        }
      }
  ...
  const stats = {
    ...
    finalizados: pedidosCompra.filter((o) => o.status === OrderStatus.COMPLETED)
      .length,
    pendentes: pedidosCompra.filter((o) => o.status === OrderStatus.IN_PROGRESS)
      .length,
    rascunhos: pedidosCompra.filter((o) => o.status === OrderStatus.DRAFT)
      .length,
      ...
      {isLoadingPedidosCompra ? (
        ...
        >
          {currentPedidosCompra.map((order, index) => {
            const partner = partnersCache.get(order.partnerId || "");
            const displayOrder = mapPurchaseOrderToDisplay(
              order,
              partner || undefined
            );
                    ...
                    <div className="flex gap-2">
                      ...
                      <motion.button
                        ...
                        onClick={() =>
                          handleDelete(
                            order.id,
                            displayOrder.partnerName || "Pedido"
                          )
                        }
                    ...
                    <React.Fragment key={order.id}>
                      ...
                      >
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                          {new Date(displayOrder.date).toLocaleDateString(
                            "pt-BR"
                          )}
                        </td>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                                  ...
                                  {(detalhes as any).transportadora && (
                                    <div className="bg-white p-4 rounded-lg shadow-sm">
                                      ...
                                      <p className="text-sm text-gray-900">
                                        {(detalhes as any).transportadora.nomeRazaoSocial}
                                      </p>
                                    </div>
                                  )}
        ...
        >
          <div className="flex items-center justify-between">
            <div className="text-sm text-gray-700">
              Mostrando {startIndex + 1} a {endIndex} de {totalPedidosCompra}{" "}
              resultados
            </div>
            ...
            <div className="flex items-center gap-2">
              ...
              <div className="flex gap-1">
                {Array.from(
                  { length: Math.ceil(totalPedidosCompra / itemsPerPage) },
                  (_, i) => i + 1
                )
                  .filter((page) => {
              ...
              <Button
                ...
                onClick={() =>
                  setCurrentPage((prev) =>
                    Math.min(
                      prev + 1,
                      Math.ceil(totalPedidosCompra / itemsPerPage)
                    )
                  )
                }
                disabled={
                  currentPage === Math.ceil(totalPedidosCompra / itemsPerPage)
                }
              >
