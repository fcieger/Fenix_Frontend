...
export default function DashboardComprasPage() {
  ...
  const [graficoComprasPorStatus, setGraficoComprasPorStatus] = useState<
    PurchasesByStatusChartData[]
  >([]);
  ...
  const [comprasPorComprador, setComprasPorComprador] = useState<
    PurchasesByBuyer[]
  >([]);
  ...
  useEffect(() => {
    const loadDashboardData = async () => {
      ...
      try {
        ...
        setError(
          error.message || "Erro ao carregar dados do dashboard de compras"
        );
      } finally {
        setLoading(false);
      }
            ...
            <div className="flex items-center">
              ...
              <div className="ml-4">
                ...
                <p className="text-2xl font-bold text-gray-900">
                  {metrics.totalPurchasesInPeriod}
                </p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
          ...
          <div className="bg-white rounded-lg shadow p-6">
            <h3 className="text-lg font-semibold text-gray-900 mb-6">
              ...
              dataInicioCustom &&
              dataFimCustom
                ? `Compras - ${new Date(dataInicioCustom).toLocaleDateString(
                    "pt-BR"
          ...
          <div className="bg-white rounded-lg shadow p-6">
            <h3 className="text-lg font-semibold text-gray-900 mb-6">
              ...
              {filtroPeriodo === "5dias" &&
                "Compras por Status - Últimos 5 Dias"}
              {filtroPeriodo === "30dias" &&
                "Compras por Status - Últimos 30 Dias"}
              {filtroPeriodo === "90dias" &&
                "Compras por Status - Últimos 90 Dias"}
              {filtroPeriodo === "personalizado" &&
              dataInicioCustom &&
              dataFimCustom
                ? `Compras por Status - ${new Date(
                    dataInicioCustom
                ...
                : filtroPeriodo !== "hoje" &&
                  ...
                  filtroPeriodo !== "30dias" &&
                  filtroPeriodo !== "90dias" &&
                  "Compras por Status - Período Selecionado"}
            </h3>
              ...
              {topFornecedores.length > 0 ? (
                <div className="space-y-4">
                  {topFornecedores.map((fornecedor, index) => (
                    <div
                      key={`fornecedor-${index}-${fornecedor.id || "sem-id"}-${
                      ...
                      <div className="flex items-center">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-gray-500">
                            {fornecedor.totalPurchases} compras
                          </p>
                        </div>
                      </div>
              ...
              {comprasPorComprador.length > 0 ? (
                <div className="space-y-4">
                  {comprasPorComprador.map((comprador, index) => (
                    <div
                      key={`comprador-${index}-${comprador.id || "sem-id"}-${
                        comprador.name || "sem-nome"
                      }`}
                      className="flex items-center justify-between p-3 bg-gray-50 rounded-lg"
                    >
                      <div className="flex items-center">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-gray-500">
                            {comprador.totalPurchases} compras
                          </p>
                        </div>
                      </div>
                      <div className="text-right">
