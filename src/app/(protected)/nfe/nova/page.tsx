...
export default function NovaNotaFiscalPage() {
  ...
  const { user, token, activeCompanyId, isAuthenticated, isLoading } =
    useAuth();
  const [activeTab, setActiveTab] = useState("dados-gerais");
  const [saving, setSaving] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [showDetailedErrors, setShowDetailedErrors] = useState(false);
  const [nfeSalva, setNfeSalva] = useState<{
    id: string;
    status: string;
  } | null>(null);
  ...
  const [configuracoesDisponiveis, setConfiguracoesDisponiveis] = useState<
    ConfiguracaoNfeResponse[]
  >([]);
  ...
  const [showTransportadoraDropdown, setShowTransportadoraDropdown] =
    useState(false);
  const [showDestinatarioDropdown, setShowDestinatarioDropdown] =
    useState(false);
  ...
  const [informacoesComplementares, setInformacoesComplementares] =
    useState("");
  ...
  useEffect(() => {
    if (configuracaoNfeId) {
      ...
      const configuracaoSelecionada = configuracoesDisponiveis.find(
        (config) => config.id === configuracaoNfeId
      );
  ...
  const carregarCadastros = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const transportadorasData = data.filter(
  ...
  const handleSearchCadastros = (query: string) => {
    ...
    if (!query) {
      ...
      setTransportadoras(
        cadastros.filter((c: any) => c.tiposCliente?.transportadora)
      );
      return;
    }
  ...
  const carregarNaturezasOperacao = async () => {
    ...
    try {
      const response = await fetch(
        `${API_CONFIG.BASE_URL}/api/natureza-operacao`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
            "Content-Type": "application/json",
      ...
      if (response.ok) {
        ...
        if (Array.isArray(data) && data.length > 0 && !naturezaOperacao) {
          console.log(
            "🎯 Selecionando primeira natureza automaticamente:",
            data[0].id
          );
        ...
        console.error(
          "Erro na resposta da API de naturezas:",
          response.status,
          response.statusText
        );
  ...
  const carregarConfiguracoesNFe = async () => {
    ...
    try {
      ...
      if (configuracoes.length > 0 && !configuracaoNfeId) {
        console.log(
          "🎯 Selecionando primeira configuração automaticamente:",
          configuracoes[0].id
        );
  ...
  const carregarConfiguracaoNatureza = async (naturezaId: string) => {
    ...
    try {
      const response = await fetch(
        `${API_CONFIG.BASE_URL}/api/natureza-operacao/${naturezaId}/configuracao-estados`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
            "Content-Type": "application/json",
      ...
      if (response.ok) {
        ...
        if (data && data.length > 0) {
          const configuracaoUF =
            data.find((config: any) => config.uf === ufDestino) || data[0];
        ...
        console.error(
          "Erro na resposta da API de configurações:",
          response.status,
          response.statusText
        );
        setConfiguracaoNatureza(null);
    ...
    try {
      ...
      if (destinatarioSelecionado?.enderecos?.length > 0) {
        const enderecoPrincipal =
          destinatarioSelecionado.enderecos.find((e: any) => e.principal) ||
          destinatarioSelecionado.enderecos[0];
      ...
      console.log(
        "🔍 VALIDAÇÃO UF - Verificando se natureza está habilitada para UF:",
        ufDestinoAtual
      );

      // Buscar configurações da natureza para validar UF
      const configuracoes = await fetch(
        `${API_CONFIG.BASE_URL}/api/natureza-operacao/${naturezaOperacaoId}/configuracao-estados`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
            "Content-Type": "application/json",
      ...
      if (!configuracaoUF || !configuracaoUF.habilitado) {
        ...
        console.log(
          "⚠️ VALIDAÇÃO UF - UF do destinatário não habilitada, usando configuração de fallback:",
          configuracaoFallback.uf
        );
        setUfErrorMessage(
        ...
        console.log(
          "✅ VALIDAÇÃO UF - Natureza habilitada para UF:",
          ufDestinoAtual
        );
      }
      ...
      const response = await fetch(
        `${API_CONFIG.BASE_URL}/api/taxes/calcular`,
        {
          method: "POST",
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  const filteredProdutos = produtosDisponiveis.filter(
    (produto) =>
      produto.descricao
        ?.toLowerCase()
        .includes(searchProdutoQuery.toLowerCase()) ||
      produto.codigo
        ?.toLowerCase()
        .includes(searchProdutoQuery.toLowerCase()) ||
      produto.categoria
  ...
  const adicionarProduto = () => {
    ...
    const novoProduto: NFeItem = {
      ...
      valorTotal:
        (produtoAtual.quantidade || 0) * (produtoAtual.valorUnitario || 0) -
        (produtoAtual.valorDesconto || 0),
    };
  ...
  const handleDestinatarioSelect = (cadastro: any) => {
    ...
    if (temIE) {
      ...
      const novoIndicador =
        indicadorIE === "2" ? "CONTRIBUINTE_ISENTO" : "CONTRIBUINTE_ICMS";
  ...
  const validateNFe = () => {
    ...
    produtos.forEach((produto, index) => {
      if (!produto.codigo?.trim()) {
        errors[`produto_${index}_codigo`] = `Código do produto ${
          index + 1
        } é obrigatório`;
      }
      if (!produto.descricao?.trim()) {
        errors[`produto_${index}_descricao`] = `Descrição do produto ${
          index + 1
        } é obrigatória`;
      }
      if (produto.quantidade <= 0) {
        errors[`produto_${index}_quantidade`] = `Quantidade do produto ${
          index + 1
        } deve ser maior que zero`;
      }
      if (produto.valorUnitario <= 0) {
        errors[`produto_${index}_valorUnitario`] = `Valor unitário do produto ${
          index + 1
        } deve ser maior que zero`;
      }
    });
    ...
    if (errorCount > 0) {
      ...
      console.log(
        "✅ VALIDAÇÃO PASSOU: Todos os campos obrigatórios estão preenchidos"
      );
    }
  ...
  const validateUF = () => {
    ...
    if (
      ufOrigem === ufDestino &&
      configuracaoNatureza?.localDestinoOperacao === "interestadual"
    ) {
  ...
  const validateNaturezaForUF = async () => {
    ...
    try {
      const configuracoes = await fetch(
        `${API_CONFIG.BASE_URL}/api/natureza-operacao/${naturezaOperacao}/configuracao-estados`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
            "Content-Type": "application/json",
  ...
  const atualizarDuplicata = (
    id: string,
    campo: keyof Duplicata,
    valor: any
  ) => {
    setDuplicatas(
      duplicatas.map((d) => (d.id === id ? { ...d, [campo]: valor } : d))
    );
  };
  ...
  const handleSalvarRascunho = async () => {
    ...
    try {
      ...
      if (configuracoes.length === 0) {
        throw new Error(
          "Nenhuma configuração NFe ativa encontrada. Configure uma configuração NFe primeiro."
        );
      }
      ...
      if (naturezas.length === 0) {
        throw new Error(
          "Nenhuma natureza de operação encontrada. Configure uma natureza de operação primeiro."
        );
      }
      const naturezaOperacaoSelecionada =
        naturezas.find((n) => n.id === naturezaOperacao) || naturezas[0];
      ...
      const nfeData = {
        ...
        destinatarioIndicadorIE: (() => {
          ...
          if (!valoresValidos.includes(valor)) {
            console.warn(
              "⚠️ Valor inválido para destinatarioIndicadorIE:",
              valor,
              "usando NAO_CONTRIBUINTE"
            );
        ...
        dataEmissao: dataEmissao
          ? new Date(dataEmissao).toISOString()
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-gray-600">
                Preencha os dados abaixo para emitir uma NFe
              </p>
            </div>
          </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  ...
                  <div className="lg:col-span-3">
                    ...
                    >
                      <SelectTrigger
                        className={
                          validationErrors.tipoOperacao ? "border-red-500" : ""
                        }
                      >
                        ...
                        {isLoadingNaturezas ? (
                          ...
                          naturezasOperacao.map((natureza) => (
                            <SelectItem key={natureza.id} value={natureza.id}>
                              {natureza.nome}{" "}
                              {natureza.cfop && `(CFOP: ${natureza.cfop})`}
                            </SelectItem>
                          ))
                        ) : (
                  ...
                  <div className="lg:col-span-3">
                        ...
                        <SelectValue
                          placeholder={
                            loadingConfiguracoes
                              ? "Carregando..."
                              : "Selecione uma configuração NFe"
                          }
                        />
                      </SelectTrigger>
                      <SelectContent>
                        {loadingConfiguracoes ? (
                          ...
                          configuracoesDisponiveis.map((config) => (
                            <SelectItem key={config.id} value={config.id}>
                              {config.descricaoModelo} - {config.modelo} Série{" "}
                              {config.serie}
                            </SelectItem>
                          ))
                        ) : (
                    ...
                    {errorNumero && (
                      <p className="text-sm text-red-600 mt-1">
                        Erro ao gerar número: {errorNumero}
                      </p>
                    )}
                  </div>
                  ...
                  <div className="flex items-center space-x-2">
                    <Checkbox
                      ...
                      onCheckedChange={(checked) =>
                        setConsumidorFinal(checked as boolean)
                      }
                    />
                  ...
                  <div>
                    <Label htmlFor="indicadorPresenca">
                      Indicador de Presença *
                    </Label>
                    <Select
                      value={indicadorPresenca}
                      onValueChange={setIndicadorPresenca}
                    >
                      <SelectTrigger>
                        <SelectValue />
                      </SelectTrigger>
                      <SelectContent>
                        ...
                        <SelectItem value="TELEATENDIMENTO">
                          Teleatendimento
                        </SelectItem>
                        <SelectItem value="NFCe_ENTREGA_DOMICILIO">
                          NFC-e em operação com entrega
                        </SelectItem>
                        <SelectItem value="NFCe_PRESENCIAL_FORA_ESTABELECIMENTO">
                          NFC-e presencial fora do estabelecimento
                        </SelectItem>
            ...
            <Card>
              ...
              <CardContent>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  <div>
                    ...
                    <div className="relative">
                      <Input
                        ...
                        value={
                          destinatarioSelecionado
                            ? destinatarioSelecionado.nomeRazaoSocial ||
                              destinatarioSelecionado.nomeFantasia
                            : ""
                        }
                        onChange={(e) => {
                          if (!destinatarioSelecionado) {
                            ...
                            {isLoadingCadastros ? (
                              ...
                              filteredCadastros.map((cadastro) => (
                                <button
                                  key={cadastro.id}
                                  onClick={() =>
                                    handleDestinatarioSelect(cadastro)
                                  }
                    ...
                    {destinatarioSelecionado && (
                      <div className="mt-2 flex items-center gap-2">
                        <Button
                          ...
                          size="sm"
                          onClick={() =>
                            handleEditDestinatario(destinatarioSelecionado.id)
                          }
                  ...
                  <div>
                    <Label htmlFor="destCnpjCpf">
                      {destTipo === "cnpj" ? "CNPJ" : "CPF"} *
                    </Label>
                    <Input
                      id="destCnpjCpf"
                      value={destCnpjCpf}
                  ...
                  <div>
                    ...
                    <Select
                      value={destIndicadorIE}
                      onValueChange={setDestIndicadorIE}
                    >
                      <SelectTrigger>
                        <SelectValue />
                      </SelectTrigger>
                      <SelectContent>
                        <SelectItem value="CONTRIBUINTE_ICMS">
                          Contribuinte ICMS
                        </SelectItem>
                        <SelectItem value="CONTRIBUINTE_ISENTO">
                          Isento
                        </SelectItem>
                  ...
                  <div>
                    <Label htmlFor="destCodigoMunicipio">
                      Código do Município
                    </Label>
                    <Input
                        ...
                        >
                          <div className="p-3 border-b border-gray-100">
                            <Input
                              ...
                              onChange={(e) =>
                                handleSearchProdutos(e.target.value)
                              }
                              className="w-full"
                    ...
                    {produtoSelecionado && (
                      <div className="mt-2 flex items-center gap-2">
                        <Button
                          ...
                          size="sm"
                          onClick={() =>
                            handleEditProduto(produtoSelecionado.id)
                          }
                          className="flex items-center gap-1"
                        >
                          <Edit className="w-3 h-3" />
                  ...
                  <div className="lg:col-span-2">
                    ...
                    <div className="relative">
                      <Input
                        ...
                        className={`pr-12 ${
                          produtoSelecionado ? "bg-gray-100" : ""
                        }`}
                      />
                      <Search className="absolute right-3 top-1/2 transform -translate-y-1/2 w-4 h-4 text-gray-400" />
                    ...
                    <Table>
                      ...
                      <TableBody>
                        {produtos.map((produto) => {
                          const natureza = naturezasOperacao.find(
                            (n) => n.id === produto.naturezaOperacao
                          );
                          return (
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
                ...
                <div className="bg-gray-50 p-4 rounded-lg">
                  ...
                  <p className="text-xl font-semibold text-orange-600">
                    {formatCurrency(impostosCalc?.totais?.totalImpostos || 0)}
                  </p>
                </div>
                <div className="bg-purple-50 p-4 rounded-lg border border-purple-200">
                  <p className="text-sm text-purple-600">Total da Nota</p>
                  <p className="text-2xl font-bold text-purple-700">
                    {formatCurrency(valorTotalNota)}
                  </p>
                </div>
              </div>
                      ...
                      {(
                        ...
                        <tr key={idx} className="border-t">
                          ...
                          <td className="py-2 pr-4">
                            {row.nome || produtos[idx]?.descricao}
                          </td>
                          <td className="py-2 pr-4">
              ...
              <div className="space-y-6">
                ...
                <div className="bg-gray-50 p-6 rounded-lg border border-gray-200">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      <Label htmlFor="modalidadeFrete">
                        Modalidade do Frete
                      </Label>
                      <Select
                        value={modalidadeFrete}
                        onValueChange={setModalidadeFrete}
                      >
                        <SelectTrigger>
                          <SelectValue placeholder="Selecione a modalidade" />
                        </SelectTrigger>
                        <SelectContent>
                          <SelectItem value="POR_CONTA_EMITENTE">
                            Por conta do emitente
                          </SelectItem>
                          <SelectItem value="POR_CONTA_DESTINATARIO">
                            Por conta do destinatário
                          </SelectItem>
                          <SelectItem value="POR_CONTA_TERCEIROS">
                    ...
                    <div className="flex items-center space-x-2">
                      <Checkbox
                        ...
                        onCheckedChange={(checked) =>
                          setIncluirFreteTotal(checked as boolean)
                        }
                      />
                      <Label htmlFor="incluirFreteTotal">
                            ...
                            <button
                              ...
                              onClick={() => {
                                ...
                                if (
                                  ...
                                  transportadora.enderecos.length > 0
                                ) {
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setVolumePesoLiquido(Number(e.target.value))
                        }
                        className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-purple-500"
                        placeholder="0.000"
                      />
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setVolumePesoBruto(Number(e.target.value))
                        }
                        className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-purple-500"
                        placeholder="0.000"
                      />
                    ...
                    <div>
                      ...
                      <input
                        ...
                        value={volumeVolume}
                        onChange={(e) =>
                          setVolumeVolume(Number(e.target.value))
                        }
                        className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-purple-500"
                        placeholder="0.000"
                      />
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setVolumeQuantidade(Number(e.target.value))
                        }
                        className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-purple-500"
                        placeholder="1"
                      />
                  ...
                  <div>
                    ...
                    >
                      ...
                      <SelectContent>
                        ...
                        <SelectItem value="CARTAO_LOJA">
                          Crédito Loja
                        </SelectItem>
                        <SelectItem value="VALE_ALIMENTACAO">
                          Vale Alimentação
                        </SelectItem>
                        <SelectItem value="VALE_REFEICAO">
                          Vale Refeição
                        </SelectItem>
                        <SelectItem value="VALE_PRESENTE">
                          Vale Presente
                        </SelectItem>
                        <SelectItem value="VALE_COMBUSTIVEL">
                          Vale Combustível
                        </SelectItem>
                        ...
                        <SelectItem value="SEM_PAGAMENTO">
                          Sem Pagamento
                        </SelectItem>
                        <SelectItem value="OUTROS">Outros</SelectItem>
                      </SelectContent>
                  ...
                  <>
                    ...
                    {duplicatas.length > 0 ? (
                      ...
                      <div className="text-center py-12 text-gray-500">
                        ...
                        <p className="text-sm">
                          Clique no botão acima para adicionar parcelas
                        </p>
                      </div>
                    )}
                ...
                <div className="space-y-6">
                  ...
                  <div>
                    ...
                    <Textarea
                      ...
                      onChange={(e) =>
                        setInformacoesComplementares(e.target.value)
                      }
                  ...
                  <div>
                    <Label htmlFor="informacoesFisco">
                  ...
                  <div className="p-4 bg-blue-50 border border-blue-200 rounded-lg">
                    ...
                    <ul className="text-sm text-blue-800 space-y-1 list-disc list-inside">
                      <li>
                        As informações complementares são de livre preenchimento
                        e aparecem no DANFE
                      </li>
                      <li>
                        As informações para o fisco são sigilosas e não aparecem
                        no DANFE
                      </li>
                      <li>
                        O número do pedido ajuda na rastreabilidade da operação
                      </li>
                    </ul>
                  </div>
                </div>
