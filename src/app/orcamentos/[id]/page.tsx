...
import { obterOrcamento, criarOrcamento, atualizarOrcamento, recalcularImpostos } from '../../../services/orcamentos';
import { criarPedidoVendaFromOrcamento, listarPedidosVenda } from '../../../services/pedidos-venda';
...
import ConvertOrcamentoModal from '@/components/vendas/ConvertOrcamentoModal';
...
function OrcamentoFormPage() {
      ...
      try {
        if (id && id !== 'novo') {
          ...
          setFormData({
            ...
            vendedor: orcamento.vendedorId || '',
            transportadora: orcamento.transportadoraId || '',
            consumidorFinal: orcamento.consumidorFinal ?? false,
            indicadorPresenca: orcamento.indicadorPresenca || '2',
            formaPagamento: orcamento.formaPagamentoId || '',
            parcelamento: orcamento.parcelamento || '90 dias',
            estoque: orcamento.localEstoqueId || '',
            pedido: orcamento.numero || '',
            nfe: orcamento.serie || '',
            numeroPedidoCotacao: orcamento.numeroPedidoCotacao || '',
            dataPrevisao: orcamento.dataPrevisaoEntrega 
              ? (typeof orcamento.dataPrevisaoEntrega === 'string' 
                  ? orcamento.dataPrevisaoEntrega.split('T')[0] 
                  : new Date(orcamento.dataPrevisaoEntrega).toISOString().split('T')[0])
              : '',
            dataEmissao: orcamento.dataEmissao 
              ? (typeof orcamento.dataEmissao === 'string' 
                  ? orcamento.dataEmissao.split('T')[0] 
                  : new Date(orcamento.dataEmissao).toISOString().split('T')[0])
              : new Date().toISOString().split('T')[0],
            dataValidade: orcamento.dataValidade 
              ? (typeof orcamento.dataValidade === 'string' 
            ...
            despesas: Number(orcamento.despesas) || 0,
            incluirFreteTotal: orcamento.incluirFreteTotal ?? false,
            placaVeiculo: orcamento.placaVeiculo || '',
            ufPlaca: orcamento.ufPlaca || '',
            rntc: orcamento.rntc || '',
            pesoLiquido: Number(orcamento.pesoLiquido) || 0,
            pesoBruto: Number(orcamento.pesoBruto) || 0,
            volume: Number(orcamento.volume) || 0,
            especie: orcamento.especie || '',
            marca: orcamento.marca || '',
            numeracao: orcamento.numeracao || '',
            quantidadeVolumes: Number(orcamento.quantidadeVolumes) || 1,
            naturezaOperacao: orcamento.naturezaOperacaoPadraoId || '',
            prazoPagamento: orcamento.prazoPagamentoId || '',
            observacoes: orcamento.observacoes || ''
          });

          // Preencher itens - mapear para o formato esperado pelo model.itens
          const itensMapped = (orcamento.itens || []).map((it: any) => ({
            id: it.id,
            companyId: it.companyId || orcamento.companyId,
            produtoId: it.produtoId || null,
            naturezaOperacaoId: it.naturezaOperacaoId || orcamento.naturezaOperacaoPadraoId || '',
            codigo: it.codigo || '',
            nome: it.nome || '',
          ...
          if (orcamento.clienteId) {
            ...
            if ((orcamento as any).cliente) {
              setClienteSelecionado((orcamento as any).cliente);
            } else {
              // Fallback: buscar na lista de cadastros
              const c = cadastros.find((c: any) => c.id === orcamento.clienteId);
              if (c) setClienteSelecionado(c);
            }
          }
          if (orcamento.vendedorId) {
            if ((orcamento as any).vendedor) {
              setVendedorSelecionado((orcamento as any).vendedor);
            } else {
              const v = cadastros.find((v: any) => v.id === orcamento.vendedorId);
              if (v) setVendedorSelecionado(v);
            }
          }
          if (orcamento.transportadoraId) {
            if ((orcamento as any).transportadora) {
              setTransportadoraSelecionada((orcamento as any).transportadora);
            } else {
              const t = cadastros.find((t: any) => t.id === orcamento.transportadoraId);
          ...
          if (orcamento.prazoPagamentoId) {
            if ((orcamento as any).prazoPagamento) {
              ...
              const p = prazosPagamento.find((p: any) => p.id === orcamento.prazoPagamentoId);
          ...
          if (orcamento.formaPagamentoId) {
            if ((orcamento as any).formaPagamento) {
              ...
              const f = formasPagamento.find((f: any) => f.id === orcamento.formaPagamentoId);
  ...
  async function salvar() {
    ...
    try {
      ...
      const modelSincronizado: Orcamento = {
        ...
        numeroPedidoCotacao: formData.numeroPedidoCotacao || model.numeroPedidoCotacao,
      ...
      if (isNovo) {
        ...
        openSuccess({
          ...
          onClose: () => {
            router.replace(`/orcamentos/${saved.id}`);
          }
        });
      } else {
  ...
  const handleInputChange = (field: string, value: any) => {
    ...
    if (field === 'cliente') {
      ...
      setModel(prev => ({ ...prev, dataValidade: value ? new Date(value).toISOString() : null }));
  ...
  const handleConverterParaVenda = () => {
    if (model.status !== 'ganho') {
      error('Erro', 'Apenas orçamentos com status "Ganho" podem ser convertidos em pedidos de venda.');
      return;
    }
  ...
  const handleConfirmConversion = async (ajustes?: any) => {
    ...
    setIsSalvando(true);
    try {
      const pedidoGerado = await criarPedidoVendaFromOrcamento(model.id, ajustes);
      
      openSuccess({
        title: 'Pedido de Venda Gerado',
        message: `Pedido de venda criado com sucesso! Número: ${pedidoGerado.numero || pedidoGerado.id?.slice(0, 8)}`,
        onOk: () => {
          router.push(`/vendas/${pedidoGerado.id}`);
        }
      });
      ...
      error('Erro', err?.response?.data?.message || 'Erro ao gerar pedido de venda. Tente novamente.');
    } finally {
      setIsSalvando(false);
    }
  };
              ...
              <div className="flex items-center space-x-3">
                ...
                <div>
                  ...
                  <p className="text-green-700 mt-1">
                    Este orçamento foi ganho. Você pode gerar um pedido de venda a partir dele.
                  </p>
                </div>
              </div>
              <Button
                  ...
                  {pedidosGerados.map((pedido) => {
                    ...
                    const getStatusBadge = (status: string) => {
                      const statusConfig: { [key: string]: { bg: string; text: string } } = {
                        ...
                        'em_preparacao': { bg: 'bg-blue-100', text: 'text-blue-800' },
                        ...
                        <div className="flex items-center justify-between">
                          <div className="flex-1">
                            <div className="flex items-center space-x-3 mb-2">
                              <button
                                onClick={() => router.push(`/vendas/${pedido.id}`)}
                            ...
                            <div className="grid grid-cols-2 gap-4 text-sm">
                              <div className="flex items-center text-gray-600">
                                ...
                                <span>Emissão: {formatDate(pedido.dataEmissao)}</span>
                              </div>
                              <div className="flex items-center text-gray-600">
                                <DollarSignIcon className="w-4 h-4 mr-2" />
                                <span className="font-semibold text-gray-900">
                                  {formatCurrency(Number(pedido.totalGeral || 0))}
                                </span>
                              </div>
                            </div>
                          </div>
