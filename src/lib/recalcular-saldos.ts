...
export async function recalcularTodosSaldos() {
  ...
  return await transaction(async (client) => {
    ...
    const contasResult = await client.query('SELECT id, saldo_inicial FROM contas_financeiras');
    const contas = contasResult.rows;
    
    console.log(`📊 Encontradas ${contas.length} contas para recalcular`);
    
    for (const conta of contas) {
      console.log(`\n🔍 Processando conta ${conta.id}...`);
...
export async function recalcularSaldoConta(contaId: string) {
  console.log(`🔄 Recalculando saldo da conta ${contaId}...`);
  
  return await transaction(async (client) => {
    // Buscar conta
    const contaResult = await client.query('SELECT id, saldo_inicial FROM contas_financeiras WHERE id = $1', [contaId]);
